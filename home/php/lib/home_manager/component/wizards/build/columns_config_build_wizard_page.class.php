...
class ColumnsConfigBuildWizardPage extends BuildWizardPage
{
    ...
    function buildForm()
    {
        ...
        for($i = 1; $i <= $row_amount; $i ++)
        {
            ...
            $this->addElement('text', 'row' . $i . 'width', Translation :: get('WidthLeft'), array(
                    'disabled' => 1,
                    'id' => 'row' . $i . 'width',
                    "size" => "50"));
            ...
            for($j = 1; $j <= $column_amount; $j ++)
            {
                ...
                $this->addElement('text', 'row' . $i . '[column' . $j . '][title]', Translation :: get('Title', null, Utilities :: COMMON_LIBRARIES), array(
                        "size" => "50"));
                $this->addElement('text', 'row' . $i . '[column' . $j . '][width]', Translation :: get('Width'), 'size="50" onchange="recalculate(' . $i . ',' . $column_amount . ')"');
                $this->addElement('text', 'row' . $i . '[column' . $j . '][blocksamount]', Translation :: get('BlockAmount'), array(
                        "size" => "50"));
