 ...
 */
class MenuManagerTreeComponent extends MenuRenderer
{
    ...
    function render()
    {
        ...
        $root_items = MenuDataManager :: get_instance()->retrieve_navigation_items($root_item_condition);
        ...
        while ($root_item = $root_items->next_result())
        {
            ...
            $subitems = MenuDataManager :: get_instance()->retrieve_navigation_items($subitem_condition);
            $count = $subitems->size();
            if ($count > 0)
            {
                $is_current = false;
