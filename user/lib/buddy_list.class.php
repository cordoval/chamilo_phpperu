...
class BuddyList
{
    ...
    function display_buddy_list_category($category, $buddies, $is_request = false, $is_normal = false)
    {
        ...
        if (! $is_request && ! $is_normal)
        {
            ...
            $toolbar_data[] = array('href' => $this->parent->get_update_buddylist_category_url($category->get_id()), 'label' => Translation :: get('Edit'), 'img' => Theme :: get_common_image_path() . 'action_edit.png');
            
            $toolbar_data[] = array('href' => $this->parent->get_delete_buddylist_category_url($category->get_id()), 'label' => Translation :: get('Delete'), 'img' => Theme :: get_common_image_path() . 'action_delete.png', 'class' => 'delete_category', 'id' => $category->get_id(), 'confirm' => true);
            
            $html[] = Utilities :: build_toolbar($toolbar_data);
        }
    ...
    function display_buddy($buddy, $is_request = false)
    {
        ...
        if (! $is_request)
        {
            $toolbar_data[] = array('href' => $this->parent->get_delete_buddylist_item_url($buddy->get_buddy_id()), 'label' => Translation :: get('DeleteUser'), 'img' => Theme :: get_common_image_path() . 'action_unsubscribe.png', 'class' => 'delete_item', 'id' => $buddy->get_buddy_id(), 'confirm' => true);
        }
        else
        {
            $toolbar_data[] = array('href' => $this->parent->get_change_buddylist_item_status_url($buddy->get_user_id(), BuddyListItem :: STATUS_NORMAL), 'label' => Translation :: get('Accept'), 'img' => Theme :: get_common_image_path() . 'action_setting_true.png', 'class' => 'accept_buddy', 'id' => $buddy->get_user_id());
    ...
    function display_buddy_list_footer()
    {
        ...
        $toolbar_data[] = array('href' => $this->parent->get_create_buddylist_category_url(), 'label' => Translation :: get('AddCategory'), 'img' => Theme :: get_common_image_path() . 'action_add.png');
        
        $toolbar_data[] = array('href' => $this->parent->get_create_buddylist_item_url(), 'label' => Translation :: get('AddUser'), 'img' => Theme :: get_common_image_path() . 'action_subscribe.png');
