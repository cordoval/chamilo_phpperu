...
class UserManagerUserDetailComponent extends UserManager implements AdministrationComponent
{
    ...
    function display_groups($user)
    {
        ...
        else
        {
            ...
            {
                $url = '<a href="' . $gm->get_link(array(
                        GroupManager :: PARAM_ACTION => GroupManager :: ACTION_VIEW_GROUP,
    ...
    function get_action_bar($user)
    {
        ...
        $action_bar->add_common_action(new ToolbarItem(Translation :: get('Edit', null, Utilities :: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_edit.png', $this->get_user_editing_url($user), ToolbarItem :: DISPLAY_ICON_AND_LABEL));

        $action_bar->add_common_action(new ToolbarItem(Translation :: get('Delete', null, Utilities :: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_delete.png', $this->get_user_delete_url($user), ToolbarItem :: DISPLAY_ICON_AND_LABEL));

        $action_bar->add_common_action(new ToolbarItem(Translation :: get('ViewQuota'), Theme :: get_common_image_path() . 'action_browser.png', $this->get_url(array(
                UserManager :: PARAM_ACTION => UserManager :: ACTION_VIEW_QUOTA,
                'user_id' => $user->get_id())), ToolbarItem :: DISPLAY_ICON_AND_LABEL));

        $action_bar->add_tool_action(new ToolbarItem(Translation :: get('VersionQuota'), Theme :: get_common_image_path() . 'action_statistics.png', $this->get_user_quota_url($user), ToolbarItem :: DISPLAY_ICON_AND_LABEL));

        $action_bar->add_tool_action(new ToolbarItem(Translation :: get('ManageRightsTemplates', null, 'rights'), Theme :: get_common_image_path() . 'action_rights.png', $this->get_manage_user_rights_url($user), ToolbarItem :: DISPLAY_ICON_AND_LABEL));

        $action_bar->add_tool_action(new ToolbarItem(Translation :: get('LoginAsUser'), Theme :: get_common_image_path() . 'action_login.png', $this->get_change_user_url($user), ToolbarItem :: DISPLAY_ICON_AND_LABEL));
