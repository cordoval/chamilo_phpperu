...
class DatabaseUserDataManager extends Database implements UserDataManagerInterface
{
    ...
    function retrieve_version_type_quota($user, $type)
    {
        ...
        {
            $user_quotum = $this->retrieve_object(UserQuota :: get_table_name(), $condition, array(), UserQuota :: CLASS_NAME);
    ...
    function delete_buddy_list_category($buddy_list_category)
    {
        ...
        $query = 'UPDATE ' . $this->escape_table_name('buddy_list_item') . ' SET ' . $this->escape_column_name(BuddyListItem :: PROPERTY_CATEGORY_ID) . '=0 WHERE' . $this->escape_column_name(BuddyListItem :: PROPERTY_CATEGORY_ID) . '=' . $this->quote($buddy_list_category->get_id());
