...
(function($)
{
    ...
    //    replace_attributes - (String|Function) Same as the above but for attributes
    //      since they don't support all types of content that regular text nodes do.
    //      If not set, replace will be used.
    ...
    $.fn.replaceText = function(search, replace, replace_attributes, text_only)
    {
        return this
                .each(function()
                {
                    ...
                    if (node)
                    {
                        ...
                        do
                        {
                            ...
                            if (node.attributes !== null)
                            {
                                for ( var i = 0; i < node.attributes.length; i++)
                                {
                                    var attribute = node.attributes[i],
                                        ...
                                        attributeReplacement = (replace_attributes ? replace_attributes : replace),
                                        newAttributeValue = attributeValue.replace(search, attributeReplacement);

                                    if (newAttributeValue !== attributeValue)
                                    {
                                        node.setAttribute(attribute.nodeName, newAttributeValue);
                                    }
                                }
                            }
                            ...
                            if (node.nodeType === 3)
                            {
                                ...
                                // Only replace text if the new value is
                                // actually different!
                                if (new_val !== val)
                                {
                                    ...
                                    {
                                        // The new value contains HTML, set it
                                        // in a slower but far more
                                        // robust way.
                                        $(node).before(new_val);

                                        // Don't remove the node yet, or the
                                        // loop will lose its place.
                                        remove.push(node);
                                    }
                                    else
                                    {
                                        // The new value contains no HTML, so it
                                        // can be set in this
