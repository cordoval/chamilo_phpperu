...
class MediawikiStringUtils
{
    ...
    static function delimiterReplaceCallback($startDelim, $endDelim, $callback, $subject, $flags = '')
    {
        ...
        while ($inputPos < strlen($subject) && preg_match("!($encStart)|($encEnd)!S$flags", $subject, $m, PREG_OFFSET_CAPTURE, $inputPos))
        {
            $tokenOffset = $m[0][1];
            if ($m[1][0] != '')
            {
            ...
            elseif ($tokenType == 'end')
            {
                if ($foundStart)
                {
                    ...
                    $output .= call_user_func($callback, array(
                            substr($subject, $outputPos, $tokenOffset + $tokenLength - $outputPos), substr($subject, $contentPos, $tokenOffset - $contentPos)));
                    $foundStart = false;
                }
                else
                {
