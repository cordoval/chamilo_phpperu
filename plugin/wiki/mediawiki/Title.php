...
class MediaWikiTitle
{
    ...
    public static function newFromIDs($ids)
    {
        ...
        $res = $dbr->select('page', array('page_namespace', 'page_title'), 'page_id IN (' . $dbr->makeList($ids) . ')', __METHOD__);
...
//            }
    ...
    private function getTitleProtection()
    {
        ...
        $dbr = wfGetDB(DB_SLAVE);
        $res = $dbr->select('protected_titles', '*', array('pt_namespace' => $this->getNamespace(), 'pt_title' => $this->getDBkey()), __METHOD__);
    ...
    public function updateTitleProtection($create_perm, $reason, $expiry)
    {
        ...
        if ($create_perm == implode(',', $this->getRestrictions('create')) && $expiry == $this->mRestrictionsExpiry['create'])
        {
            // No change
            return true;
        }
        ...
        if ($create_perm != '')
        {
            $dbw->replace('protected_titles', array(array('pt_namespace', 'pt_title')), array(
                    'pt_namespace' => $namespace, 'pt_title' => $title, 'pt_create_perm' => $create_perm, 'pt_timestamp' => Block :: encodeExpiry(wfTimestampNow(), $dbw), 'pt_expiry' => $encodedExpiry,
    ...
    public function deleteTitleProtection()
    {
        ...
        $dbw->delete('protected_titles', array('pt_namespace' => $this->getNamespace(), 'pt_title' => $this->getDBkey()), __METHOD__);
    }
    ...
    public function getSubpages($limit = -1)
    {
        ...
        if ($limit > - 1)
            $options['LIMIT'] = $limit;
        return $this->mSubpages = TitleArray :: newFromResult($dbr->select('page', array('page_id', 'page_namespace', 'page_title', 'page_is_redirect'), $conds, __METHOD__, $options));
    }
    ...
    public function isDeleted()
    {
        ...
        {
            $dbr = wfGetDB(DB_SLAVE);
            $n = $dbr->selectField('archive', 'COUNT(*)', array('ar_namespace' => $this->getNamespace(), 'ar_title' => $this->getDBkey()), __METHOD__);
            if ($this->getNamespace() == NS_FILE)
            {
    ...
    public function isDeletedQuick()
    {
        ...
        $dbr = wfGetDB(DB_SLAVE);
        $deleted = (bool) $dbr->selectField('archive', '1', array('ar_namespace' => $this->getNamespace(), 'ar_title' => $this->getDBkey()), __METHOD__);
...
//        while (true);
    ...
    public function getLinksTo($options = array(), $table = 'pagelinks', $prefix = 'pl')
    {
        ...
        $res = $db->select(array('page', $table), array('page_namespace', 'page_title', 'page_id', 'page_len', 'page_is_redirect'), array("{$prefix}_from=page_id", "{$prefix}_namespace" => $this->getNamespace(), "{$prefix}_title" => $this->getDBkey()), __METHOD__, $options);
    ...
    public function getBrokenLinksFrom()
    {
        ...
        $dbr = wfGetDB(DB_SLAVE);
        $res = $dbr->select(array('page', 'pagelinks'), array('pl_namespace', 'pl_title'), array('pl_from' => $this->getArticleId(), 'page_namespace IS NULL'), __METHOD__, array(), array(
                'page' => array('LEFT JOIN', array('pl_namespace=page_namespace', 'pl_title=page_title'))));
    ...
    public function moveTo(&$nt, $auth = true, $reason = '', $createRedirect = true)
    {
        ...
        if ($protected)
        {
            ...
            $dbw->insertSelect('page_restrictions', 'page_restrictions', array(
                    'pr_page' => $redirid, 'pr_type' => 'pr_type', 'pr_level' => 'pr_level', 'pr_cascade' => 'pr_cascade', 'pr_user' => 'pr_user', 'pr_expiry' => 'pr_expiry'), array('pr_page' => $pageid), __METHOD__, array(
                    'IGNORE'));
            # Update the protection log
            $log = new LogPage('protect');
    ...
    private function moveOverExistingRedirect(&$nt, $reason = '', $createRedirect = true)
    {
        ...
        $dbw->update('page',
			/* SET */ array('page_touched' => $dbw->timestamp($now), 'page_namespace' => $nt->getNamespace(), 'page_title' => $nt->getDBkey(), 'page_latest' => $nullRevId),
			/* WHERE */ array('page_id' => $oldid), $fname);
    ...
    private function moveToNewTitle(&$nt, $reason = '', $createRedirect = true)
    {
        ...
        # Rename page entry
        $dbw->update('page',
			/* SET */ array('page_touched' => $now, 'page_namespace' => $nt->getNamespace(), 'page_title' => $nt->getDBkey(), 'page_latest' => $nullRevId),
			/* WHERE */ array('page_id' => $oldid), $fname);
        $nt->resetArticleID($oldid);
    ...
    public function moveSubpages($nt, $auth = true, $reason = '', $createRedirect = true)
    {
        ...
        foreach ($subpages as $oldSubpage)
        {
            ...
            if ($oldSubpage->getArticleId() == $this->getArticleId())
                ...
                // don't move it twice
                continue;
            $newPageName = preg_replace('#^' . preg_quote($this->getDBKey(), '#') . '#', $nt->getDBKey(), $oldSubpage->getDBKey());
    ...
    public function isSingleRevRedirect()
    {
        $dbw = wfGetDB(DB_MASTER);
        # Is it a redirect?
        $row = $dbw->selectRow('page', array('page_is_redirect', 'page_latest', 'page_id'), $this->pageCond(), __METHOD__, array('FOR UPDATE'));
        ...
        $row = $dbw->selectField(array('page', 'revision'), 'rev_id', array('page_namespace' => $this->getNamespace(), 'page_title' => $this->getDBkey(), 'page_id=rev_page', 'page_latest != rev_id'), __METHOD__, array(
                'FOR UPDATE'));
    ...
    public function isValidMoveTarget($nt)
    {
        ...
        if (preg_match("/\\[\\[\\s*([^\\]\\|]*)]]/", $text, $m))
        {
            ...
            if (! is_object($redirTitle) || ($redirTitle->getPrefixedDBkey() != $this->getPrefixedDBkey() && $redirTitle->getPrefixedDBkey() != $nt->getPrefixedDBkey()))
            {
    ...
    {
        $db = ($flags & GAID_FOR_UPDATE) ? wfGetDB(DB_MASTER) : wfGetDB(DB_SLAVE);
        return $db->selectField('revision', 'rev_id', array('rev_page' => $this->getArticleId($flags), 'rev_id < ' . intval($revId)), __METHOD__, array('ORDER BY' => 'rev_id DESC'));
    }
    ...
    {
        $db = ($flags & GAID_FOR_UPDATE) ? wfGetDB(DB_MASTER) : wfGetDB(DB_SLAVE);
        return $db->selectField('revision', 'rev_id', array('rev_page' => $this->getArticleId($flags), 'rev_id > ' . intval($revId)), __METHOD__, array('ORDER BY' => 'rev_id'));
    }
    ...
    public function getFirstRevision($flags = 0)
    {
        ...
        if (! $pageId)
            return NULL;
        $row = $db->selectRow('revision', '*', array('rev_page' => $pageId), __METHOD__, array('ORDER BY' => 'rev_timestamp ASC', 'LIMIT' => 1));
        if (! $row)
        {
            return NULL;
        }
    ...
    public function getEarliestRevTime()
    {
        ...
        if ($this->exists())
        {
            $min = $dbr->selectField('revision', 'MIN(rev_timestamp)', array('rev_page' => $this->getArticleId()), __METHOD__);
    ...
    {
        $dbr = wfGetDB(DB_SLAVE);
        return $dbr->selectField('revision', 'count(*)', 'rev_page = ' . intval($this->getArticleId()) . ' AND rev_id > ' . intval($old) . ' AND rev_id < ' . intval($new), __METHOD__);
    }
    ...
    public function getNotificationTimestamp($user = NULL)
    {
        ...
        $dbr = wfGetDB(DB_SLAVE);
        $this->mNotificationTimestamp[$uid] = $dbr->selectField('watchlist', 'wl_notificationtimestamp', array('wl_namespace' => $this->getNamespace(), 'wl_title' => $this->getDBkey(), 'wl_user' => $user->getId()), __METHOD__);
        return $this->mNotificationTimestamp[$uid];
    }
    ...
    public function trackbackURL()
    {
        ...
        return "$wgServer$wgScriptPath/trackback$wgScriptExtension?article=" . htmlspecialchars(urlencode($this->getPrefixedDBkey()));
    }
