...
(function()
{
	...
	var names = [ 'play', 'loop', 'menu', 'quality', 'scale', 'salign', 'wmode', 'bgcolor', 'base', 'chamilodailymotionvars', 'allowScriptAccess',
		'allowFullScreen' ];
	...
	CKEDITOR.dialog.add( 'chamilodailymotion', function( editor )
	{
		var makeObjectTag = !editor.config.chamilodailymotionEmbedTagOnly,
			makeEmbedTag = editor.config.chamilodailymotionAddEmbedTag || editor.config.chamilodailymotionEmbedTagOnly;

		var previewPreloader,
			previewAreaHtml = '<div>' + CKEDITOR.tools.htmlEncode( editor.lang.common.preview ) +'<br>' +
			'<div id="ChamilodailymotionPreviewLoader" style="display:none"><div class="loading">&nbsp;</div></div>' +
			'<div id="ChamilodailymotionPreviewBox"></div></div>';

		return {
			title : editor.lang.chamilodailymotion.title,
			minWidth : 420,
			minHeight : 310,
			onShow : function()
			{
				...
				// Try to detect any embed or object tag that has dailymotion parameters.
				var fakeImage = this.getSelectedElement();
				if ( fakeImage && fakeImage.getAttribute( '_cke_real_element_type' ) && fakeImage.getAttribute( '_cke_real_element_type' ) == 'chamilodailymotion' )
				{
			...
			onOk : function()
			{
				...
				// Apply or remove chamilodailymotion parameters.
				var extraStyles = {};
				this.commitContent( objectNode, embedNode, paramMap, extraStyles );

				// Refresh the fake image.
				var newFakeImage = editor.createFakeElement( objectNode || embedNode, 'cke_chamilodailymotion', 'chamilodailymotion', true );
									...
									[
										{
											...
											required : true,
											validate : CKEDITOR.dialog.validate.notEmpty( editor.lang.chamilodailymotion.validateSrc ),
											setup : loadValue,
											commit : commitValue,
											onLoad : function()
											{
							...
							[
								{
									...
									id : 'width',
									style : 'width:190px',
									label : editor.lang.chamilodailymotion.width,
									validate : CKEDITOR.dialog.validate.integer( editor.lang.chamilodailymotion.validateWidth ),
								...
								},
								{
									...
									disabled : true,
									style : 'width:190px',
									label : editor.lang.chamilodailymotion.height,
									validate : CKEDITOR.dialog.validate.integer( editor.lang.chamilodailymotion.validateHeight ),
									setup : function( objectNode, embedNode, paramMap, fakeImage )
									{
										loadValue.apply( this, arguments );
										if ( fakeImage )
