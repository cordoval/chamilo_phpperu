...
class LastRss
{
	// TODO: Implement PEAR RSS parser package
	...
	function LastRss($url)
	{
		...
		$this->date_format = 'd F Y, H:i:s';
		
		$this->cache_dir = dirname(__FILE__).'/../../cache';
		// Default is 3600 seconds = 1 hour
		$this->cache_time = 3600;
		
		// Feed url as passed on from the module
		$this->feed_url = $url;
	}
	
	function get_default_cp()
	{
	...
	function get_items_limit()
	{
		return $this->items_limit;
	}
	
	function get_strip_html()
	{
		return $this->strip_html;
	}
	...
	function set_default_cp($default_cp)
	{
		$this->default_cp = $default_cp;
	}
	
	function set_cdata($cdata)
	{
		$this->cdata =$cdata ;
	}
	...
	function set_items_limit($items_limit)
	{
		$this->items_limit = $items_limit;
	}
	
	function set_strip_html($strip_html)
	{
	...
	function set_feed_url($feed_url)
	{
	...
	function get_feed_content()
	{
		...
		if ($this->cache_dir != '')
		{
			$cache_file = $this->cache_dir . '/rsscache_' . md5($this->feed_url);
			...
			else
			{
				...
				$result = $this->parse_feed_content();
	...
	// -------------------------------------------------------------------
	private function parse_feed_content()
	{
		// Open and load RSS file
		if ($f = @fopen($this->feed_url, 'r'))
		{
			$rss_content = '';
			while (!feof($f))
			{
