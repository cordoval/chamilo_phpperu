...
class GroupManagerImporterComponent extends GroupManager implements AdministrationComponent
{
    ...
    function run()
    {
        ...
        if ($form->validate())
        {
            ...
            $this->redirect(Translation :: get($success ? 'GroupXMLProcessed' : 'GroupXMLNotProcessed') . '<br />' . $form->get_failed_elements(), ($success ? false : true), array(
                    Application :: PARAM_ACTION => GroupManager :: ACTION_IMPORT));
        }
        else
        {
            $this->display_header();
