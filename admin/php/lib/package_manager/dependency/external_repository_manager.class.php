...
class ExternalRepositoryManagerPackageDependency extends PackageDependency
{
    ...
    function as_html()
    {
        ...
        return Translation :: get('TypeName', null, ExternalRepositoryManager :: get_namespace($this->get_id())) . ', ' . Translation :: get('Version', array(), Utilities :: COMMON_LIBRARIES) . ': ' . $version['_content'];
    }

    function check()
    {
        $version = $this->get_version();
        $message = Translation :: get('DependencyCheckExternalRepositoryManager') . ': ' . $this->as_html() . ' ' . Translation :: get('Found', array(), Utilities :: COMMON_LIBRARIES) . ': ';
        ...
        else
        {
            ...
            if (! $application_version)
            {
                ...
                $this->logger->add_message($message);
                $this->logger->add_message(Translation :: get('DependencyExternalRepositoryManagerWrongVersion'), MessageLogger :: TYPE_WARNING);
                return false;
            }
            else
            {
                ...
                {
                    $message .= '--' . Translation :: get('InactiveExternalRepositoryManager', array(), Utilities :: COMMON_LIBRARIES) . '--';
                    $this->logger->add_message($message);
                    $this->logger->add_message(Translation :: get('DependencyActivateExternalRepositoryManagerWarning'), MessageLogger :: TYPE_WARNING);
                }
                else
                {
