...
class VideoConferencingManagerPackageDependency extends PackageDependency
{
    ...
    function as_html()
    {
        ...
        return Translation :: get('TypeName', null, VideoConferencingManager :: get_namespace($this->get_id())) . ', ' . Translation :: get('Version', array(), Utilities :: COMMON_LIBRARIES) . ': ' . $version['_content'];
    }

    function check()
    {
        $version = $this->get_version();
        $message = Translation :: get('DependencyCheckVideoConferencingManager') . ': ' . $this->as_html() . ' ' . Translation :: get('Found', array(), Utilities :: COMMON_LIBRARIES) . ': ';
        ...
        else
        {
            ...
            if (! $application_version)
            {
                ...
                $this->logger->add_message($message);
                $this->logger->add_message(Translation :: get('DependencyVideoConferencingManagerWrongVersion'), MessageLogger :: TYPE_WARNING);
                return false;
            }
            else
            {
                ...
                {
                    $message .= '--' . Translation :: get('InactiveVideoConferencingManager', array(), Utilities :: COMMON_LIBRARIES) . '--';
                    $this->logger->add_message($message);
                    $this->logger->add_message(Translation :: get('DependencyActivateVideoConferencingManagerWarning'), MessageLogger :: TYPE_WARNING);
                }
                else
                {
