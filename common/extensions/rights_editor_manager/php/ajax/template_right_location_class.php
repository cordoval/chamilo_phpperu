...
if (isset($right_template) && isset($right) && isset($location))
{
    ...
    //$value = $this->is_allowed($id, $right_template->get_id(), $locked_parent->get_id());
    ...
    {
        $value = RightsUtilities :: get_rights_template_right_location($right, $right_template, $location->get_id());

        if (! $value)
        {
            if ($location->inherits())
            {
                $inherited_value = RightsUtilities :: is_allowed_for_rights_template($right_template, $right, $location);
