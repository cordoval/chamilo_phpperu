...
class SoundcloudExternalRepositoryObject extends ExternalRepositoryObject
{
    ...
    function get_tags_string($include_links = true)
    {
        ...
        foreach ($this->get_tags() as $tag)
        {
            if ($include_links)
            {
                $tags[] = '<a href="http://www.soundcloud.com/photos/tags/' . $tag['text'] . '">' . $tag['display'] . '</a>';
            }
            else
            {
                $tags[] = $tag['display'];
    ...
    {
        return Theme :: get_common_image('external_repository/soundcloud/licenses/license_' . $this->get_license_id(), 'png', $this->get_license_name(), $this->get_license_url(), ToolbarItem :: DISPLAY_ICON);
    }
    ...
    static function get_possible_licenses()
    {
        ...
        $licenses[7] = array('id' => 7, 'name' => 'No known copyright restrictions', 'url' => 'http://www.soundcloud.com/commons/usage/');
