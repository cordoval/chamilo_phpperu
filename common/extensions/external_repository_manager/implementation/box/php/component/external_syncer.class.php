...
class BoxExternalRepositoryManagerExternalSyncerComponent extends BoxExternalRepositoryManager
{
    ...
    function synchronize_external_repository_object(ExternalRepositoryObject $external_object)
    {
        ...
        $values = array();
        $values[ExternalInstance :: PROPERTY_ID] = $external_object->get_id();
        $values[ExternalInstance :: PROPERTY_TITLE] = trim(html_entity_decode(strip_tags($content_object->get_title())));
        $values[ExternalInstance :: PROPERTY_DESCRIPTION] = trim(html_entity_decode(strip_tags($content_object->get_description())));
