...
class Dropbox_API
{
    ...
    public function copy($from, $to, $root = null)
    {
        ...
        if (is_null($root))
            $root = $this->root;
        $response = $this->oauth->fetch('http://api.dropbox.com/0/fileops/copy', array(
                'from_path' => $from,
                'to_path' => $to,
                'root' => $root));
    ...
    public function createFolder($path, $root = null)
    {
     ...
     // Making sure the path starts with a /
        ...
        $response = $this->oauth->fetch('http://api.dropbox.com/0/fileops/create_folder', array(
                'path' => $path,
                'root' => $root), 'POST');
        return json_decode($response['body'], true);
    ...
    public function delete($path, $root = null)
    {
        ...
        if (is_null($root))
            $root = $this->root;
        $response = $this->oauth->fetch('http://api.dropbox.com/0/fileops/delete', array(
                'path' => $path,
                'root' => $root));
    ...
    public function move($from, $to, $root = null)
    {
        ...
        if (is_null($root))
            $root = $this->root;
        $response = $this->oauth->fetch('http://api.dropbox.com/0/fileops/move', array(
                'from_path' => $from,
                'to_path' => $to,
                'root' => $root));
    ...
    public function getThumbnail($path, $size = 'small', $root = null)
    {
        ...
        if (is_null($root))
            $root = $this->root;
        $response = $this->oauth->fetch('http://api-content.dropbox.com/0/thumbnails/' . $root . '/' . ltrim($path, '/'), array(
                'size' => $size));
