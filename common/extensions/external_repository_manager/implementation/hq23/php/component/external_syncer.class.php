...
class Hq23ExternalRepositoryManagerExternalSyncerComponent extends Hq23ExternalRepositoryManager
{
    ...
    function synchronize_external_repository_object(ExternalRepositoryObject $external_object)
    {
        ...
        if ($this->get_external_repository_connector()->update_external_repository_object($values))
        {
            ...
            if ($synchronization_data->update())
            {
                ...
                $this->redirect(Translation :: get('ObjectUpdated', array('ContentObject' => Translation :: get('ContentObject')), Utilities :: COMMON_LIBRARIES), false, $parameters, array(ExternalRepositoryManager :: PARAM_EXTERNAL_REPOSITORY, ExternalRepositoryManager :: PARAM_EXTERNAL_REPOSITORY_MANAGER_ACTION));
            }
            else
            {
                $parameters = $this->get_parameters();
                ...
                $this->redirect(Translation :: get('ObjectFailedUpdated', array('ContentObject' => Translation :: get('ContentObject')), Utilities :: COMMON_LIBRARIES), true, $parameters);
            }
        }
        else
        {
            ...
            $parameters[ExternalRepositoryManager :: PARAM_EXTERNAL_REPOSITORY_MANAGER_ACTION] = ExternalRepositoryManager :: ACTION_VIEW_EXTERNAL_REPOSITORY;
            $parameters[ExternalRepositoryManager :: PARAM_EXTERNAL_REPOSITORY_ID] = $external_object->get_id();
            $this->redirect(Translation :: get('ObjectFailedUpdated', array('ExternalRepository' => Translation :: get('ExternalRepository')), Utilities :: COMMON_LIBRARIES), true, $parameters);
        }
    }
}
?>