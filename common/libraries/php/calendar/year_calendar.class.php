 ...
 * calendar tool.
 */
class YearCalendar extends CalendarTable
{
    /**
    ...
    function build_tables()
    {
        ...
        {
            $time = mktime($date_parts['hours'], $date_parts['minutes'], $date_parts['seconds'], $month, $date_parts['mday'], $date_parts['year']);

            $navigation = new HTML_Table('class="calendar_navigation"');
            $navigation->updateCellAttributes(0, 0, 'style="text-align: center;"');
            $navigation->setCellContents(0, 0, Translation :: get(date('F', $time) . 'Long')/* . ' ' . date('Y', $time)*/);
    ...
    public function add_calendar_navigation($url_format)
    {
        ...
        $navigation->setCellContents(0, 0, '<a href="' . htmlspecialchars(str_replace('-TIME-', $prev, $url_format)) . '"><img src="' . htmlspecialchars(Theme :: get_common_image_path()) . 'action_prev.png" style="vertical-align: middle;" alt="&lt;&lt;"/></a> ');
        $navigation->setCellContents(0, 1, date('Y', $this->get_display_time()));
        $navigation->setCellContents(0, 2, ' <a href="' . htmlspecialchars(str_replace('-TIME-', $next, $url_format)) . '"><img src="' . htmlspecialchars(Theme :: get_common_image_path()) . 'action_next.png" style="vertical-align: middle;" alt="&gt;&gt;"/></a> ');
    ...
    public function toHtml()
    {
        ...
        for($month = 1; $month <= 12; $month ++)
        {
            ...
            $this->setCellAttributes($row, $column, array(
