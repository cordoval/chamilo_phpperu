...
class QtiImportStrategyGeneric extends QtiImportStrategyBase
{
    ...
    public function get_general_feedbacks(ImsXmlReader $item)
    {
        ...
        {
            if ($null_feedbacks[$i] == $correct_feedbacks[$i] && $correct_feedbacks[$i] == $incorrect_feedbacks[$i] && ! empty($correct_feedbacks[$i]))
            {
    ...
    {
        if (! $interaction->is_sliderInteraction() && ! $interaction->is_textEntryInteraction() && ! ! $interaction->is_extendedTextEntryInteraction())
        {
            return false;
        }
    ...
    {
        if (! $interaction->is_sliderInteraction() && ! $interaction->is_textEntryInteraction() && ! $interaction->is_extendedTextInteraction())
        {
            return false;
        }
        if ($this->head()->is_formula($answer))
    ...
    protected function get_score_formula_from_if(ImsXmlReader $if, $response_id, $score_id)
    {
        ...
        if ($condition->is_equal())
        {
            ...
            if ($condition->is_patternMatch() && $condition->get_patternMatch()->get_variable()->identifier == $response_id)
            {
                return $condition;
            }
        return false;
