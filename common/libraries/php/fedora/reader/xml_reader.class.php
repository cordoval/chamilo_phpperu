...
class FedoraXmlReader implements IteratorAggregate
{
    ...
    public static function get_empty_reader()
    {
        ...
        {
            self :: $empty_reader = new FedoraXmlReaderEmpty();
        }
    ...
    {
        if ($current instanceof FedoraXmlReader)
        {
    ...
    public function load($item)
    {
        if (is_string($item))
        {
            ...
            if ($item instanceof FedoraXmlReader)
            {
    ...
    {
        $result = $this->doc->documentElement->getAttribute('xmlns');
        $result = $result ? $result : $this->doc->documentElement->namespaceURI;
        return $result;
    }
    ...
    public function query($path, $context = null)
    {
        if (substr($path, 0, 1) != '.')
        {
            ...
            if (is_null($context))
            {
                ...
                if ($context instanceof FedoraXmlReader)
                {
     ...
     * @return FedoraXmlReaderTopDownIterator
     */
    public function getIterator()
    {
        return new FedoraXmlReaderTopDownIterator($this);
    }
 ...
 *
 */
class FedoraXmlReaderTopDownIterator implements Iterator
{
    private $reader = null;
    /**
     *
    ...
    public function __construct(FedoraXmlReader $reader)
    {
        $this->reader = $reader;
    ...
    protected function get_next_up(DOMNode $node)
    {
        if (empty($node->parentNode))
        {
            ...
            if ($node->parentNode == $this->start)
            {
                $result = false;
            }
            else
                if (! empty($node->parentNode->nextSibling))
                {
