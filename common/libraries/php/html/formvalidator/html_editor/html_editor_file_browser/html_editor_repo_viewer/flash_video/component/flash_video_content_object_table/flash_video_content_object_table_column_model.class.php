<?php
namespace common\libraries;

use common\extensions\repo_viewer\ContentObjectTableColumnModel;

/**
 * $Id: content_object_table_column_model.class.php 191 2009-11-13 11:50:28Z chellee $
 * @package application.common.repo_viewer.component.content_object_table
 */
require_once Path :: get_common_extensions_path() . 'repo_viewer/php/component/content_object_table/content_object_table_column_model.class.php';
/**
 * This class represents a column model for a publication candidate table
 */
class FlashVideoContentObjectTableColumnModel extends ContentObjectTableColumnModel
{
}
?>