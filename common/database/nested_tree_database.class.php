...
class NestedTreeDatabase extends Database
{
    ...
    function move($node, $new_parent_id = 0, $new_previous_id = 0, $condition = null)
    {
        ...
        if ($new_previous_id)
        {
            ...
            $new_previous = $this->retrieve_node($node, $new_previous_id);
        ...
        else
        {
            ...
            $new_parent = $this->retrieve_node($node, $new_parent_id);
        ...
        // Return false if this failed
        $node = $this->retrieve_node($node, $node->get_id());
        ...
        if ($new_previous_id)
        {
            $temp = $this->retrieve_node($node, $new_previous_id);
        ...
        {
            $temp = $this->retrieve_node($node, $new_parent_id);
