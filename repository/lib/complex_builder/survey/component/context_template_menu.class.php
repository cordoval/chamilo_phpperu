...
class SurveyContextTemplateMenu extends HTML_Menu
{
     ...
     *                           root.
     */
    function SurveyContextTemplateMenu($current_template, $root_co ,$url_format = '?go=build_complex&application=repository&builder_action=configure_context&root_lo=%s&template_id=%s', $include_root = true, $show_complete_tree = false, $hide_current_template = false)
    {
        $this->root_lo = $root_lo;
        ...
        {
            $condition = new EqualityCondition(SurveyContextTemplate :: PROPERTY_PARENT_ID, 0);
            $template = SurveyContextDataManager :: get_instance()->retrieve_survey_context_templates($condition, null, 1, new ObjectTableOrder(SurveyContextTemplate :: PROPERTY_CONTEXT_TYPE))->next_result();
    ...
    function get_menu()
    {
        ...
        if (! $include_root)
        {
            return $this->get_menu_items($template->get_id());
        }
        else
        {
            $menu = array();
            ...
            $sub_menu_items = $this->get_menu_items($template->get_id());
            ...
            $menu_item[OptionsMenuRenderer :: KEY_ID] = $template->get_id();
            $menu[$template->get_id()] = $menu_item;
            return $menu;
        }
    }
    ...
    private function get_menu_items($parent_id = 0)
    {
        ...
        while ($template = $templates->next_result())
        {
            ...
            if (! ($template_id == $current_template->get_id() && $hide_current_template))
            {
                $menu_item = array();
                $menu_item['title'] = $template->get_name();
                $menu_item['url'] = $this->get_url($template->get_id());
                
                if ($template->is_parent_of($current_category) || $template->get_id() == $current_category->get_id() || $show_complete_tree)
                {
                    if ($template->has_children())
                    {
                        $menu_item['sub'] = $this->get_menu_items($template->get_id());
                    }
                }
                else
                {
    ...
    function get_url($template)
    {
        ...
        return htmlentities(sprintf($this->urlFmt, $this->root_lo,$template));
    }
    ...
    private function get_home_url($template)
    {
    	...
    	return htmlentities(sprintf($this->urlFmt, $this->root_lo));
    }
