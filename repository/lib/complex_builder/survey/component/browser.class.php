...
class SurveyBuilderBrowserComponent extends SurveyBuilderComponent
{
    ...
    function get_creation_links($lo, $types = array())
    {
        ...
        foreach ($types as $type)
        {
            if ($type == 'survey_page')
            {
                $url = $this->get_url(array(ComplexBuilder :: PARAM_BUILDER_ACTION => SurveyBuilder :: ACTION_CREATE_SURVEY_ITEM, ComplexBuilder :: PARAM_TYPE => $type, ComplexBuilder :: PARAM_ROOT_LO => $this->get_root_lo()->get_id(), ComplexBuilder :: PARAM_CLOI_ID => ($this->get_cloi() ? $this->get_cloi()->get_id() : null), 'publish' => Request :: get('publish')));
