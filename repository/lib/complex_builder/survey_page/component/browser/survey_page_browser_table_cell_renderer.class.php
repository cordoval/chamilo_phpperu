...
class SurveyPageBrowserTableCellRenderer extends ComplexBrowserTableCellRenderer
{
    ...
    function render_cell($column, $cloi)
    {
        ...
        $ref_lo = $lo;
        if ($lo->get_type() == 'survey_page_item')
        {
    ...
    protected function get_modification_links($cloi, $lo)
    {
        ...
        if ($lo->get_type() == 'survey_page_item')
        {
            ...
            if ($this->count > 1)
            {
                $routing = $cloi->get_routing();
                if (! empty($routing))
                {
                    $additional_items[] = array('href' => $this->browser->get_routing_url($cloi->get_id()), 'label' => Translation :: get('EditRouting'), 'img' => Theme :: get_common_image_path() . 'action_edit_prerequisites.png');
                }
                else
                {
                    $additional_items[] = array('href' => $this->browser->get_routing_url($cloi->get_id()), 'label' => Translation :: get('BuildRouting'), 'img' => Theme :: get_common_image_path() . 'action_build_prerequisites.png');
                }
            }
