...
class WikiDisplayWikiHistoryComponent extends WikiDisplay
{
    ...
    function run()
    {
        ...
        $this->complex_id = Request :: get(ComplexDisplay :: PARAM_COMPLEX_CONTENT_OBJECT_ITEM_ID);
        ...
        $this->action_bar = $this->get_parent()->get_toolbar($this, Request :: get('pid'), $this->get_root_content_object(), $this->complex_id);
        ...
        if (count($versions) >= 2)
        {
             ...
             */
            $form = ContentObjectForm :: factory(ContentObjectForm :: TYPE_COMPARE, $wiki_page, 'compare', 'post', $this->get_url(array(Tool :: PARAM_ACTION => Request :: get('tool') == 'learning_path' ? 'view_clo' : 'view', 'display_action' => 'history', 'pid' => $this->get_root_content_object()->get_id(), ComplexDisplay :: PARAM_COMPLEX_CONTENT_OBJECT_ITEM_ID => $this->complex_id)), array('version_data' => $version_data));
