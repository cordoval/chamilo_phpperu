...
class GlossaryViewerTableCellRenderer extends DefaultContentObjectTableCellRenderer
{
    ...
    function get_actions($glossary_item)
    {
        ...
        {
            $actions[] = array('href' => $this->browser->get_complex_content_object_item_update_url($glossary_item->get_id()), 'label' => Translation :: get('Edit'), 'img' => Theme :: get_common_image_path() . 'action_edit.png');
        }
        
        if ($this->browser->is_allowed(DELETE_RIGHT))
        {
            $actions[] = array('href' => $this->browser->get_complex_content_object_item_delete_url($glossary_item->get_id()), 'label' => Translation :: get('Delete'), 'img' => Theme :: get_common_image_path() . 'action_delete.png');
        }
