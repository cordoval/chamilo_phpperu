...
class LearningPathBuilderDeleterComponent extends LearningPathBuilderComponent
{
    ...
    function run()
    {
        ...
        if (! empty($ids))
        {
            ...
            $this->redirect(Translation :: get($message), $failures ? true : false, array(ComplexBuilder :: PARAM_BUILDER_ACTION => ComplexBuilder :: ACTION_BROWSE_CLO, ComplexBuilder :: PARAM_CLOI_ID => $parent_cloi, ComplexBuilder :: PARAM_ROOT_CONTENT_OBJECT => $root, 'publish' => Request :: get('publish')));
        }
        else
        {
