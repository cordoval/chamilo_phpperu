 ...
 */
class VideoConferencing extends RepositoryDataClass
{
    ...
    const PROPERTY_TYPED_VIDEO_CONFERENCING_ID = 'typed_video_conferencing_id';
    ...
    function set_typed_external_repository_id($id)
    {
        ...
        {
            $this->set_default_property(self :: PROPERTY_TYPED_VIDEO_CONFERENCING_ID, $id);
        }
    }
    ...
    {
        return $this->get_default_property(self :: PROPERTY_TYPED_VIDEO_CONFERENCING_ID, DataClass :: NO_UID);
    }
    ...
    static function get_default_property_names($extended_property_names = array())
    {
        ...
        $extended_property_names[] = self :: PROPERTY_TYPED_VIDEO_CONFERENCING_ID;
    ...
    function get()
    {
        if ($this->is_identified())
        {
            ...
            $result_set = $dm->retrieve_video_conferencing_condition($condition);
    ...
    function get_typed_repository_object()
    {
        ...
        {
            $type = $this->get_type();
            if (VideoConferencing :: require_once_video_conferencing_class_file($type))
            {
                $class_name = 'VideoConferencing' . Utilities :: underscores_to_camelcase($type);

                $typed_repository = new $class_name();

                $typed_repository->set_id($this->get_typed_video_conferencing_id());
     ...
     * @return boolean Indicates wether the object properties could be retrieved from the datasource
     */
    function get_by_typed_video_conferencing_id()
    {
        $typed_video_conferencing_id = $this->get_typed_video_conferencing_id();

        if (isset($typed_video_conferencing_id) && $typed_video_conferencing_id != DataClass :: NO_UID)
        {
            $dm = RepositoryDataManager :: get_instance();

            $condition = new EqualityCondition(self :: PROPERTY_TYPED_VIDEO_CONFERENCING_ID, $typed_video_conferencing_id);
     ...
     */
    public static function retrieve_video_conferencing_condition($condition = null)
    {
        if (! isset($condition))
        {
            /*
        ...
        while ($object = $result_set->next_result())
        {
            ...
            ExternalRepository :: require_once_video_conferencing_class_file($object->get_type());
        }
