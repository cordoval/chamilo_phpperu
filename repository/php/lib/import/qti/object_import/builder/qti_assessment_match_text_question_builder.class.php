...
class QtiAssessmentMatchTextQuestionBuilder extends QtiQuestionBuilder
{
    ...
    {
        if (! class_exists('repository\content_object\assessment_match_text_question\AssessmentMatchTextQuestion') || $item->has_templateDeclaration() || count($item->list_interactions()) != 1 || ! self :: has_score($item))
        {
            return null;
        }
        ...
        if (! is_object($response))
        {
            ...
            if (! ($response instanceof ImsXmlReader))
            {
                ...
                if ($response->is_patternMatch())
                {
                    ...
                    if ($response->is_stringMatch())
                    {
                        return $response->caseSensitive == 'true';
                    }
                    else
                        if ($response->get_parent()->is_stringMatch())
                        {
