...
class QtiMatchQuestionBuilder extends QtiQuestionBuilder
{
    ...
    {
        if (! class_exists('repository\content_object\match_question\MatchQuestion') || $item->has_templateDeclaration() || count($item->list_interactions()) != 1 || ! self :: has_score($item))
        {
            return null;
        }
