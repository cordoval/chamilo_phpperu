...
/**
 * Question builder for open questions.
 *
 ...
 *
 */
class QtiAssessmentOpenQuestionBuilder extends QtiQuestionBuilder
{

    static function factory($item, $settings)
    {
        if (! class_exists('repository\content_object\assessment_open_question\AssessmentOpenQuestion') || $item->has_templateDeclaration() || ! self :: has_score($item))
        {
            return null;
        }
        ...
        foreach ($interactions as $interaction)
        {
            if ($interaction->is_extendedTextInteraction())
            {
                ...
                if ($interaction->is_uploadInteraction())
                {
                    $upload_count ++;
                }
                else
