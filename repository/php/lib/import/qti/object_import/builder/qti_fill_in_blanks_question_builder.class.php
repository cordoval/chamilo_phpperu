...
class QtiFillInBlanksQuestionBuilder extends QtiQuestionBuilder
{
    ...
    {
        if (! class_exists('repository\content_object\fill_in_blanks_question\FillInBlanksQuestion') || count($item->list_interactions()) < 2 || $item->has_templateDeclaration() || ! self :: has_score($item))
        {
            return null;
        }
