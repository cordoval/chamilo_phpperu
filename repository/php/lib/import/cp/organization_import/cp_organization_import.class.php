    ...
    protected function import_item(LearningPath $object, ImscpManifestReader $item, ObjectImportSettings $settings)
    {
        ...
        if (! empty($href))
        {
            ...
            if ($child_object = CpImport :: object_factory($resource_settings)->import_content_object())
            {
                ...
                if ($child_object instanceof ContentObject && in_array($child_object->get_type(), $types))
                {
        ...
        $process_children = count($children_item) > 0 && (empty($child_object) || ($child_object instanceof ContentObject && ! $child_object->is_complex_content_object()));
        if ($process_children)
        {
