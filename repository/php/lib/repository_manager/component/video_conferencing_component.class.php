...
/**
 * $Id: video_conferencing_repository_component.class.php 204 2009-11-13 12:51:30Z kariboe $
 * @package repository.lib.repository_manager.component
 */

class RepositoryManagerVideoConferencingComponent extends RepositoryManagerMetadataComponent
{
    ...
    function get_catalogs()
    {
        ...
        $catalogs[VideoConferencing :: CATALOG_REPOSITORY_LIST] = VideoConferencing :: retrieve_video_conferencing_condition();
     ...
     */
    function get_video_conferencing_from_param()
    {
        $repository_id = Request :: get(RepositoryManager :: PARAM_VIDEO_CONFERENCING_ID);

        if (isset($repository_id) && strlen($repository_id) > 0)
        {
            $repository = new VideoConferencing();
     ...
     *
     * @param VideoConferencing $export
    ...
    public function add_chamilo_infos($export, $objects_list)
    {
        ...
        {
            if(isset($object[BaseVideoConferencingConnector :: EXTERNAL_OBJECT_KEY][BaseVideoConferencingConnector :: OBJECT_ID]))
            {
                //$content_object = ContentObjectMetadata :: get_by_catalog_entry_values($catalog_name, $object[BaseVideoConferencingConnector :: EXTERNAL_OBJECT_KEY][BaseVideoConferencingConnector :: OBJECT_ID]);
                 ...
                 */
                $eesi = VideoConferencingSyncInfo :: get_by_external_uid_and_repository($object[BaseVideoConferencingConnector :: EXTERNAL_OBJECT_KEY][BaseVideoConferencingConnector :: OBJECT_ID], $export->get_id());
                ...
                if(isset($eesi))
                {
                    ...
                    $object[BaseVideoConferencingConnector :: CHAMILO_OBJECT_KEY] = $content_object;

//                    /*
//                     * Get the last synchronization date with the repository if exists
//                     */
//                    $eesi = VideoConferencingSyncInfo :: get_by_content_object_and_repository($content_object->get_id(), $export->get_id());

//    	            if(isset($eesi))
//    	            {
    	                $object[BaseVideoConferencingConnector :: SYNC_INFO] = $eesi;

    	                /*
    	                 * Compare dates between the repository and Chamilo
    	                 */
    	                $repository_datetime          = strtotime($object[BaseVideoConferencingConnector :: EXTERNAL_OBJECT_KEY][BaseVideoConferencingConnector :: OBJECT_MODIFICATION_DATE]);
    	                $chamilo_synchronized         = strtotime(date('Y-m-d H:i:s', strtotime($object[BaseVideoConferencingConnector :: SYNC_INFO]->get_utc_synchronized() . 'z')));
    	                $synchronized_object_datetime = strtotime($object[BaseVideoConferencingConnector :: SYNC_INFO]->get_synchronized_object_datetime());
    	                ...
    	                if($current_object_date > $synchronized_object_datetime)
    	                {
    	                     ...
    	                     */
    	                    $object[BaseVideoConferencingConnector :: SYNC_STATE] = BaseVideoConferencingConnector :: SYNC_NEWER_IN_CHAMILO;
    	                }
    	                elseif($current_object_date == $synchronized_object_datetime)
    	                {
    	                     ...
    	                     */
    	                    $object[BaseVideoConferencingConnector :: SYNC_STATE] = BaseVideoConferencingConnector :: SYNC_IDENTICAL;
    	                    ...
    	                    {
    	                        $object[BaseVideoConferencingConnector :: SYNC_STATE] = BaseVideoConferencingConnector :: SYNC_OLDER_IN_CHAMILO;
    	                    }
    	                }
    	                else
    	                {
    	                     ...
    	                     */
    	                    throw new Exception('RepositoryManagerVideoConferencingExportListObjectsComponent error: The current object date is smaller than its date of synchronization');
    	                }
//    	            }
//    	            else
//    	            {
//    	                $object[BaseVideoConferencingConnector :: SYNC_STATE] = BaseVideoConferencingConnector :: SYNC_NEVER_SYNCHRONIZED;
//    	            }
                }
                else
                {
