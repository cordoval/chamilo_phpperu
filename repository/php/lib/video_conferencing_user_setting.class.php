...
class VideoConferencingUserSetting extends DataClass
{
     ...
     * @return mixed
     */
    static function get($variable, $video_conferencing_id = null, $user_id = null)
    {
        if (is_null($video_conferencing_id) || ! is_numeric($external_repository_id))
        {
        ...
        if (! isset(self :: $settings[$video_conferencing_id][$user_id]))
        {
            self :: load($video_conferencing_id, $user_id);
        }

        return (isset(self :: $settings[$video_conferencing_id][$user_id][$variable]) ? self :: $settings[$video_conferencing_id][$user_id][$variable] : null);
    }

    static function get_all($video_conferencing_id = null, $user_id = null)
    {
        if (is_null($video_conferencing_id) || ! is_numeric($video_conferencing_id))
        {
            $video_conferencing_id = Request :: get(VideoConferencingManager :: PARAM_VIDEO_CONFERENCING);
        ...
        return self :: $settings[$video_conferencing_id][$user_id];
    }

    static function load($video_conferencing_id, $user_id)
    {
        ...
        {
            $conditions = array();
            $conditions[] = new EqualityCondition(VideoConferencingUserSetting :: PROPERTY_USER_ID, $user_id);
            $conditions[] = new EqualityCondition(VideoConferencingUserSetting :: PROPERTY_SETTING_ID, $setting->get_id());
            $condition = new AndCondition($conditions);

            $user_settings = RepositoryDataManager :: get_instance()->retrieve_video_conferencing_user_settings($condition, array(), 0, 1);
            if ($user_settings->size() == 1)
            {
                $user_setting = $user_settings->next_result();
                self :: $settings[$video_conferencing_id][$user_id][$setting->get_variable()] = $user_setting->get_value();
            }
        }
    }
}
