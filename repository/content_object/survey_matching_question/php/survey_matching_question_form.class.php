...
class SurveyMatchingQuestionForm extends ContentObjectForm
{
    ...
    protected function build_creation_form()
    {
        ...
        $this->addElement('html', ResourceManager :: get_instance()->get_resource_html(Path :: get(WEB_PATH) . 'repository/content_object/survey_matching_question/resources/javascript/survey_matching_question.js'));
    }
    ...
    function setDefaults($defaults = array ())
    {
        ...
        if ($object->get_number_of_options() != 0)
        {
            ...
            {
                $defaults[SurveyMatchingQuestionOption :: PROPERTY_VALUE][$index] = $option->get_value();
            }
    ...
    function add_answer()
    {
        ...
        foreach ($values[SurveyMatchingQuestionOption :: PROPERTY_VALUE] as $option_id => $value)
        {
    ...
    function add_options()
    {
        ...
        for($option_number = 0; $option_number < $number_of_options; $option_number ++)
        {
            ...
            if (! in_array($option_number, $_SESSION['mq_skip_options']))
            {
                ...
                $group[] = $this->create_html_editor(SurveyMatchingQuestionOption :: PROPERTY_VALUE . '[' . $option_number . ']', Translation :: get('Answer'), $html_editor_options);
                ...
                $this->addGroup($group, SurveyMatchingQuestionOption :: PROPERTY_VALUE . '_' . $option_number, null, '', false);

                $renderer->setElementTemplate('<tr id="option_' . $option_number . '" class="' . ($visual_number % 2 == 0 ? 'row_odd' : 'row_even') . '">{element}</tr>', SurveyMatchingQuestionOption :: PROPERTY_VALUE . '_' . $option_number);
                $renderer->setGroupElementTemplate('<td>{element}</td>', SurveyMatchingQuestionOption :: PROPERTY_VALUE . '_' . $option_number);
