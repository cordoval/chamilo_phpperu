 ...
 */
class SurveyMatrixQuestionForm extends ContentObjectForm
{
    ...
    function setDefaults($defaults = array ())
    {
        ...
        if ($object->get_number_of_options() != 0)
        {
            ...
            {
                $defaults[SurveyMatrixQuestionOption :: PROPERTY_VALUE][$index] = $option->get_value();
            }
    ...
    function add_answers()
    {
        ...
        foreach ($values[SurveyMatrixQuestionOption :: PROPERTY_VALUE] as $option_id => $value)
        {
            //Create the option with it corresponding match
            $options[] = new SurveyMatrixQuestionOption($value);
        }
    ...
    function update_number_of_options_and_matches()
    {
        ...
        {
            $_SESSION['mq_matrix_type'] = SurveyMatrixQuestion :: MATRIX_TYPE_RADIO;
        }
        ...
        {
            $_SESSION['mq_matrix_type'] = $_SESSION['mq_matrix_type'] == SurveyMatrixQuestion :: MATRIX_TYPE_RADIO ? SurveyMatrixQuestion :: MATRIX_TYPE_CHECKBOX : SurveyMatrixQuestion :: MATRIX_TYPE_RADIO;
        }
    ...
    function add_options()
    {
        ...
        if ($_SESSION['mq_matrix_type'] == SurveyMatrixQuestion :: MATRIX_TYPE_RADIO)
        {
            $switch_label = Translation :: get('SwitchToMultipleMatches');
            $multiple = false;
        }
        elseif ($_SESSION['mq_matrix_type'] == SurveyMatrixQuestion :: MATRIX_TYPE_CHECKBOX)
        {
        ...
        for($option_number = 0; $option_number < $number_of_options; $option_number ++)
        {
            ...
            if (! in_array($option_number, $_SESSION['mq_skip_options']))
            {
                ...
                $group[] = $this->create_html_editor(SurveyMatrixQuestionOption :: PROPERTY_VALUE . '[' . $option_number . ']', Translation :: get('Answer'), $html_editor_options);
                ...
                $this->addGroup($group, SurveyMatrixQuestionOption :: PROPERTY_VALUE . '_' . $option_number, null, '', false);

                $renderer->setElementTemplate('<tr id="option_' . $option_number . '" class="' . ($visual_number % 2 == 0 ? 'row_odd' : 'row_even') . '">{element}</tr>', SurveyMatrixQuestionOption :: PROPERTY_VALUE . '_' . $option_number);
                $renderer->setGroupElementTemplate('<td>{element}</td>', SurveyMatrixQuestionOption :: PROPERTY_VALUE . '_' . $option_number);
            }
        }
        $table_footer[] = '</tbody>';
        $table_footer[] = '</table>';
