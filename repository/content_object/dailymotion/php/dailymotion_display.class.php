...
class DailymotionDisplay extends ContentObjectDisplay {
    ...
    function get_description() {
        ...
        return str_replace(self :: DESCRIPTION_MARKER, '<div class="link_url" style="margin-top: 1em;">' . $video_element . '<br/><a href="' . htmlentities($object->get_url()) . '"'.$target.'>' . htmlentities($object->get_url()) . '</a></div>' . self :: DESCRIPTION_MARKER, $html);
    }

    function get_short_html() {
        $target = $this->get_link_target();
        $object = $this->get_content_object();
        return '<span class="content_object"><a target="' . $target . '" href="' . htmlentities($object->get_url()) . '">' . htmlentities($object->get_title()) . '</a></span>';
    }
