...
class SurveyMultipleChoiceQuestionForm extends ContentObjectForm
{
    ...
    function setDefaults($defaults = array ())
    {
        if (! $this->isSubmitted())
        {
            ...
            if ($object->get_number_of_options() != 0)
            {
                ...
                {
                    $defaults[SurveyMultipleChoiceQuestionOption :: PROPERTY_VALUE][$index] = $option->get_value();
                }
            }
            else
            {
    ...
    function add_options()
    {
        ...
        {
            $_SESSION['mc_answer_type'] = SurveyMultipleChoiceQuestion :: ANSWER_TYPE_RADIO;
        }
        if (isset($_POST['add']))
        {
        ...
        {
            $_SESSION['mc_answer_type'] = $_SESSION['mc_answer_type'] == SurveyMultipleChoiceQuestion :: ANSWER_TYPE_RADIO ? SurveyMultipleChoiceQuestion :: ANSWER_TYPE_CHECKBOX : SurveyMultipleChoiceQuestion :: ANSWER_TYPE_RADIO;
        }
        $object = $this->get_content_object();
        ...
        if ($_SESSION['mc_answer_type'] == SurveyMultipleChoiceQuestion :: ANSWER_TYPE_RADIO)
        {
            $switch_label = Translation :: get('SwitchToCheckboxes');
        }
        elseif ($_SESSION['mc_answer_type'] == SurveyMultipleChoiceQuestion :: ANSWER_TYPE_CHECKBOX)
        {
        ...
        for($option_number = 0; $option_number < $number_of_options; $option_number ++)
        {
            if (! in_array($option_number, $_SESSION['mc_skip_options']))
            {
                ...
                $group[] = $this->create_html_editor(SurveyMultipleChoiceQuestionOption :: PROPERTY_VALUE . '[' . $option_number . ']', Translation :: get('Answer'), $html_editor_options);
                ...
                $this->addGroup($group, SurveyMultipleChoiceQuestionOption :: PROPERTY_VALUE . '_' . $option_number, null, '', false);

                $renderer->setElementTemplate('<tr id="option_' . $option_number . '" class="' . ($option_number % 2 == 0 ? 'row_even' : 'row_odd') . '">{element}</tr>', SurveyMultipleChoiceQuestionOption :: PROPERTY_VALUE . '_' . $option_number);
                $renderer->setGroupElementTemplate('<td>{element}</td>', SurveyMultipleChoiceQuestionOption :: PROPERTY_VALUE . '_' . $option_number);
            }
        }
