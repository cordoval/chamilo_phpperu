...
class SurveyBuilderSubscribeContextTemplateComponent extends SurveyBuilder
{
    ...
    function run()
    {
        ...
        $form = new SubscribeContextTemplateForm(SubscribeContextTemplateForm :: TYPE_CREATE, $survey, $this->get_url(array(
                self :: PARAM_SURVEY_ID => $survey_id)), $this->get_user());

        if ($form->validate())
        {
            $form->subscribe_context_template();
            $this->redirect(Translation :: get('ContextTemplateSubscribed'), (false), array(
