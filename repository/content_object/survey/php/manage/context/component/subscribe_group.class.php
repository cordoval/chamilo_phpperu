...
//        }
        ...
        if ($form->validate())
        {
            ...
            if ($success)
            {
                $this->redirect(Translation :: get('ObjectsCreated', array('OBJECTS' => Translation::get('SurveyContextRelGroups')), Utilities::COMMON_LIBRARIES), (false), array(self :: PARAM_ACTION => self :: ACTION_VIEW_CONTEXT, self :: PARAM_CONTEXT_ID => $context_id,  self :: PARAM_CONTEXT_REGISTRATION_ID => $context_registration_id, DynamicTabsRenderer :: PARAM_SELECTED_TAB => SurveyContextManagerContextViewerComponent :: TAB_GROUPS));
            }
            else
            {
                $this->redirect(Translation :: get('ObjectsNotCreated', array('OBJECTS' => Translation::get('SurveyContextRelGroups')), Utilities::COMMON_LIBRARIES), (true), array(self :: PARAM_ACTION => self :: ACTION_VIEW_CONTEXT, self :: PARAM_CONTEXT_ID => $context_id,  self :: PARAM_CONTEXT_REGISTRATION_ID => $context_registration_id, DynamicTabsRenderer :: PARAM_SELECTED_TAB => SurveyContextManagerContextViewerComponent :: TAB_GROUPS));
            }
        }
        else
        {
 ...
 function add_additional_breadcrumbs(BreadcrumbTrail $breadcrumbtrail)
    {
        ...
        $breadcrumbtrail->add(new Breadcrumb($this->get_url(array(self :: PARAM_ACTION => self :: ACTION_VIEW_CONTEXT, self :: PARAM_CONTEXT_ID => Request :: get(self :: PARAM_CONTEXT_ID), DynamicTabsRenderer :: PARAM_SELECTED_TAB => SurveyContextManagerContextViewerComponent :: TAB_GROUPS)), Translation :: get('BrowseObjects',array('OBJECTS' => Translation::get('SurveyContexts')),Utilities::COMMON_LIBRARIES)));
    }
