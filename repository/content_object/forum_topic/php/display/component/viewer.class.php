...
class ForumTopicDisplayViewerComponent extends ForumTopicDisplay
{
    ...
    function run()
    {
        $topic = $this->get_root_content_object();
        $this->retrieve_children($topic->get_id());

        $content_object = RepositoryDataManager :: get_instance()->retrieve_content_object($topic->get_id(), ForumTopic :: get_type_name());
    ...
    function retrieve_children($topic)
    {
        ...
        $children = $rdm->retrieve_complex_content_object_items(new EqualityCondition(ComplexContentObjectItem :: PROPERTY_PARENT, $topic, ComplexContentObjectItem :: get_table_name()), array(
                new ObjectTableOrder(ComplexContentObjectItem :: PROPERTY_ADD_DATE, SORT_ASC)));
        while ($child = $children->next_result())
        {
