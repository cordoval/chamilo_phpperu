    ...
    function add_options()
    {
        ...
        for($option_number = 0; $option_number < $number_of_options; $option_number ++)
        {
            ...
            if (! in_array($option_number, $_SESSION['mq_skip_options']))
            {
                ...
                $group[] = $this->create_html_editor(AssessmentMatchingQuestionOption :: PROPERTY_VALUE . '[' . $option_number . ']', Translation :: get('Answer'), $html_editor_options);
                ...
                $this->addGroup($group, AssessmentMatchingQuestionOption :: PROPERTY_VALUE . '_' . $option_number, null, '', false);

                $renderer->setElementTemplate('<tr id="option_' . $option_number . '" class="' . ($visual_number % 2 == 0 ? 'row_odd' : 'row_even') . '">{element}</tr>', AssessmentMatchingQuestionOption :: PROPERTY_VALUE . '_' . $option_number);
                $renderer->setGroupElementTemplate('<td>{element}</td>', AssessmentMatchingQuestionOption :: PROPERTY_VALUE . '_' . $option_number);
