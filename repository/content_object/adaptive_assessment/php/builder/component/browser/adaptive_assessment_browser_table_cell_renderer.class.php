...
class AdaptiveAssessmentBrowserTableCellRenderer extends ComplexBrowserTableCellRenderer
{
    ...
    function __construct($browser, $condition)
    {
        ...
        $subselect_condition = new NotCondition(new EqualityCondition(ContentObject :: PROPERTY_TYPE, AdaptiveAssessment :: get_type_name()));
            ...
            case ContentObject :: PROPERTY_TITLE :
                ...
                if ($content_object->get_type() == AdaptiveAssessment :: get_type_name())
                {
                    $title_short = '<a href="' . $this->browser->get_url(array(
                            ComplexBuilder :: PARAM_COMPLEX_CONTENT_OBJECT_ITEM_ID => $complex_content_object_item->get_id())) . '">' . $title_short . '</a>';
                }
                else
                {
    ...
    protected function get_modification_links($complex_content_object_item, $content_object)
    {
        ...
        if ($content_object->get_type() == AdaptiveAssessmentItem :: get_type_name())
        {
            ...
            {
                $toolbar->add_item(new ToolbarItem(Translation :: get('EditPrerequisites'), Theme :: get_common_image_path() . 'action_edit_prerequisites.png', $this->browser->get_prerequisites_url($complex_content_object_item->get_id()), ToolbarItem :: DISPLAY_ICON));
            }
            else
            {
                $toolbar->add_item(new ToolbarItem(Translation :: get('BuildPrerequisites'), Theme :: get_common_image_path() . 'action_build_prerequisites.png', $this->browser->get_prerequisites_url($complex_content_object_item->get_id()), ToolbarItem :: DISPLAY_ICON));
            }

            if ($this->lpi_ref_object->get_type() == Assessment :: get_type_name())
            {
                $toolbar->add_item(new ToolbarItem(Translation :: get('SetMasteryScore'), Theme :: get_common_image_path() . 'action_quota.png', $this->browser->get_mastery_score_url($complex_content_object_item->get_id()), ToolbarItem :: DISPLAY_ICON));
            }
        }

        $toolbar->add_item(new ToolbarItem(Translation :: get('Edit'), Theme :: get_common_image_path() . 'action_edit.png', $this->browser->get_complex_content_object_item_edit_url($complex_content_object_item->get_id()), ToolbarItem :: DISPLAY_ICON));
        ...
        $toolbar->add_item(new ToolbarItem(Translation :: get('ChangeParent'), Theme :: get_common_image_path() . 'action_move.png', $change_parent_url, ToolbarItem :: DISPLAY_ICON));

        $allowed = $this->check_move_allowed($complex_content_object_item);

        if ($allowed["moveup"])
        {
            $toolbar->add_item(new ToolbarItem(Translation :: get('MoveUp'), Theme :: get_common_image_path() . 'action_up.png', $moveup_url, ToolbarItem :: DISPLAY_ICON));
        }
        else
        {
        ...
        if ($allowed["movedown"])
        {
            $toolbar->add_item(new ToolbarItem(Translation :: get('MoveDown'), Theme :: get_common_image_path() . 'action_down.png', $movedown_url, ToolbarItem :: DISPLAY_ICON));
        }
        else
        {
