...
class AssessmentMatchTextQuestionForm extends ContentObjectForm
{
    ...
    protected function build_creation_form()
    {
        ...
        $this->add_html_editor(AssessmentMatchTextQuestion :: PROPERTY_HINT, Translation :: get('Hint', array(), Utilities :: get_namespace_from_object($this)), false, $html_editor_options);
        $renderer->setElementTemplate('{element}<div class="clear"></div>', AssessmentMatchTextQuestion :: PROPERTY_HINT);
        $this->addElement('category');
    }
    ...
    private function add_options()
    {
        ...
        $this->addElement('hidden', 'match_number_of_options', $_SESSION['match_number_of_options'], array(
                'id' => 'match_number_of_options'));
        ...
        for($option_number = 0; $option_number < $number_of_options; $option_number ++)
        {
            if (! in_array($option_number, $_SESSION['match_skip_options']))
            {
                ...
                $group[] = $this->createElement('textarea', "option[$option_number]", Translation :: get('Answer'), array(
                        'style' => 'width: 100%; height:' . $textarea_height));
