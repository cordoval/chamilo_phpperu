...
abstract class Dokeos185CourseDataMigrationDataClass extends Dokeos185MigrationDataClass
{
    ...
    function create_publication($object, $course_id, $user_id, $tool, $category_id = 0, $target_users = null, $target_groups = null, $show_on_homepage = 0)
    {
        ...
        $publication->set_show_on_homepage($show_on_homepage);
    ...
    function add_resources($object, $type)
    {
        $resources = $this->get_data_manager()->retrieve_resources($this->get_course(), $object->get_id(), $type);
        while($resource = $resources->next_result())
        {
            ...
            if($new_resource_id)
            {
                $object->attach_content_object($new_resource_id);
            }
        }
    }
