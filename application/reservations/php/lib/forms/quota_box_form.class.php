...
class QuotaBoxForm extends FormValidator
{
    ...
    function build_basic_form()
    {
        ...
        $this->addElement('text', QuotaBox :: PROPERTY_NAME, Translation :: get('Title', null, Utilities :: COMMON_LIBRARIES));
        $this->addRule(QuotaBox :: PROPERTY_NAME, Translation :: get('ThisFieldIsRequired', null, Utilities :: COMMON_LIBRARIES), 'required');
        $this->add_html_editor(QuotaBox :: PROPERTY_DESCRIPTION, Translation :: get('Description', null, Utilities :: COMMON_LIBRARIES), false);
    ...
    function create_quota_box()
    {
        ...
        if ($succes)
        {
            Event :: trigger('create_quota_box', ReservationsManager :: APPLICATION_NAME, array(ChangesTracker :: PROPERTY_REFERENCE_ID => $quota_box->get_id(), ChangesTracker :: PROPERTY_USER_ID => $this->user->get_id()));
        }
    ...
    function update_quota_box()
    {
        ...
        if ($succes)
        {
            Event :: trigger('update_quota_box', ReservationsManager :: APPLICATION_NAME, array(ChangesTracker :: PROPERTY_REFERENCE_ID => $quota_box->get_id(), ChangesTracker :: PROPERTY_USER_ID => $this->user->get_id()));
        }
