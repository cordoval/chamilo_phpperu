...
class ForumToolBrowserComponent extends ForumTool
{
    ...
    function create_table_forums($table, &$row, $parent)
    {
        ...
        $publications = $datamanager->retrieve_content_object_publications($condition, new ObjectTableOrder(Forum :: PROPERTY_DISPLAY_ORDER_INDEX, SORT_ASC));
        ...
        if ($this->is_allowed(EDIT_RIGHT))
        {
            ...
            else
            {
                $toolbar->add_item(new ToolbarItem(
		        		...
		        		$this->get_url(array(Tool :: PARAM_PUBLICATION_ID => $publication->get_id(), Tool :: PARAM_ACTION => Tool :: ACTION_MOVE, Tool :: PARAM_MOVE => - 1)),
		        		ToolbarItem :: DISPLAY_ICON
		        ));
            }
