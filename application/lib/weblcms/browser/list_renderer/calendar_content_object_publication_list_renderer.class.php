...
class CalendarContentObjectPublicationListRenderer extends ContentObjectPublicationListRenderer
{
    ...
    function get_calendar_events($from_time, $to_time, $limit = 0)
    {
        ...
        {
            if (method_exists($this->get_tool_browser()->get_parent(), 'convert_content_object_publication_to_calendar_event'))
            {
                $publication = $this->get_tool_browser()->get_parent()->convert_content_object_publication_to_calendar_event($publication, $from_time, $to_time);
            }
    ...
    function as_html()
    {
        ...
        $mini_month_calendar = ContentObjectPublicationListRenderer :: factory(ContentObjectPublicationListRenderer :: TYPE_MINI_MONTH, $this->get_tool_browser());
        ...
        switch ($view)
        {
            case self :: CALENDAR_DAY_VIEW :
                $calendar = ContentObjectPublicationListRenderer :: factory(ContentObjectPublicationListRenderer :: TYPE_DAY, $this->get_tool_browser());
                break;
            case self :: CALENDAR_WEEK_VIEW :
                $calendar = ContentObjectPublicationListRenderer :: factory(ContentObjectPublicationListRenderer :: TYPE_WEEK, $this->get_tool_browser());
                break;
