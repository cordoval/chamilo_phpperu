...
class WikiManagerWikiEvaluationComponent extends WikiManagerComponent
{
    function run()
    {
        ...
        if ($parameters[EvaluationManager :: PARAM_PUBLICATION_ID])
        {
    		...
    		$trail->add(new Breadcrumb($this->get_url(array(EvaluationManager :: PARAM_EVALUATION_ACTION => EvaluationManager :: ACTION_BROWSE, EvaluationManager :: PARAM_PARAMETERS => $parameter_string)), Translation :: get('BrowseEvaluations') . ' ' . $wiki_publication->get_content_object()->get_title()));
