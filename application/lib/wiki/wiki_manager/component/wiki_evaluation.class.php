...
class WikiManagerWikiEvaluationComponent extends WikiManagerComponent
{
    function run()
    {
        ...
        if ($parameters[EvaluationManager :: PARAM_PUBLICATION_ID])
        {
    		...
    		$trail->add(new Breadcrumb($this->get_url(array(EvaluationManager :: PARAM_ACTION => EvaluationManager :: ACTION_BROWSE, EvaluationManager :: PARAM_PARAMETERS => $parameter_string)), Translation :: get('Evaluations') . ' ' . $wiki_publication->get_content_object()->get_title()));
			$evaluation_manager = new EvaluationManager($this, $wiki_publication, Request :: get('action'), $trail);
        }  
        else
        {
