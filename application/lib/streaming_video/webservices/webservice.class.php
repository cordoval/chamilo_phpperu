...
$ws = new StreamingVideoWebService();
$ws->run();

class StreamingVideoWebService {
	private $uri;
	...
	function run() {
		if (strtolower($_SERVER['QUERY_STRING']) == 'wsdl') {
			...
			$facade = new StreamingVideoWebServiceFacade();
			$facade->service = $this;
		...
		//verify upload account
                ...
                foreach($segments as $segment)
                {
                    ...
                    if($clip->create())
                    {
                        ...
                        {
                            throw new StreamingVideoWebServiceException($e->getMessage());
                        }
                    }
