...
require_once dirname(__FILE__) . '/streaming_video_utilities.class.php';
...
class UploadAccount extends DataClass
{
	...
	{
                $upload_password = StreamingVideoUtilities :: create_upload_pasword();
	...
	static function get_table_name()
	{
        ...
        {
                return (defined('STREAMING_VIDEO_ACCOUNT_VALIDITY')
			? STREAMING_VIDEO_ACCOUNT_VALIDITY : self::DEFAULT_VALIDITY);
        }

        protected static function get_validity_grace()
        {
                 return (defined('STREAMING_VIDEO_ACCOUNT_VALIDITY_GRACE')
			? STREAMING_VIDEO_ACCOUNT_VALIDITY_GRACE : self::DEFAULT_VALIDITY_GRACE);
        }

        function set_homedir(){
                //TODO:what if upload_account :: username doesn't yet exist ??
                $this->set_default_property(self :: PROPERTY_HOMEDIR, Platformsetting :: get('upload_path', 'streaming_video') . '/' . $this->get_username() . '/');
        }
		...
		$account = $dm->retrieve_upload_account($user->get_username(),
			StreamingVideoUtilities :: create_upload_pasword());
