...
class SurveyPublicationBrowserTableCellRenderer extends DefaultSurveyPublicationTableCellRenderer
{
    ...
    private function get_modification_links($survey_publication)
    {
        ...
        if ($user->is_platform_admin() || $user->get_id() == $survey_publication->get_publisher())
        {
            ...
            //will be changed in editer: change in type !
            //            $toolbar->add_item(new ToolbarItem(Translation :: get('ChangeToProduction'), Theme :: get_common_image_path() . 'export_repository.png', $this->browser->get_change_test_to_production_url($survey_publication), ToolbarItem :: DISPLAY_ICON));
            

            if ($survey_publication->get_hidden())
            {
                $toolbar->add_item(new ToolbarItem(Translation :: get('Show'), Theme :: get_common_image_path() . 'action_visible_na.png', $this->browser->get_change_survey_publication_visibility_url($survey_publication), ToolbarItem :: DISPLAY_ICON));
            }
            else
            {
                $toolbar->add_item(new ToolbarItem(Translation :: get('Hide'), Theme :: get_common_image_path() . 'action_visible.png', $this->browser->get_change_survey_publication_visibility_url($survey_publication), ToolbarItem :: DISPLAY_ICON));
            }
