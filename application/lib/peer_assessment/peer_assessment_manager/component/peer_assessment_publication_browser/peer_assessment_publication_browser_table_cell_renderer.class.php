...
class PeerAssessmentPublicationBrowserTableCellRenderer extends DefaultPeerAssessmentPublicationTableCellRenderer
{
        ...
        if ($property = $column->get_name())
        {
            switch ($property)
            {
                case ContentObject :: PROPERTY_TITLE :
                    $url = $this->browser->get_url(array(PeerAssessmentManager :: PARAM_ACTION => PeerAssessmentManager :: ACTION_VIEW, PeerAssessmentDisplay :: PARAM_DISPLAY_ACTION => PeerAssessmentDisplay :: ACTION_VIEW_PEER_ASSESSMENT, PeerAssessmentManager :: PARAM_PEER_ASSESSMENT_PUBLICATION => $peer_assessment_publication->get_id()));
    ...
    private function get_modification_links($peer_assessment_publication)
    {
        ...
        if(WebApplication :: is_active('gradebook'))
        {
        	...
        	if(GradebookDatamanager :: get_instance()->retrieve_internal_item_by_publication('peer_assessment', $peer_assessment_publication->get_id()))
