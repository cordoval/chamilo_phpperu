...
class DatabaseAlexiaDataManager extends AlexiaDataManager
{
    ...
    function count_alexia_publications($condition = null)
    {
        ...
        $query .= ' JOIN ' . $rdm->escape_table_name(ContentObject :: get_table_name()) . ' AS ' . $object_alias . ' ON ' . $this->database->escape_column_name(AlexiaPublication :: PROPERTY_CONTENT_OBJECT, $publication_alias) . ' = ' . $rdm->escape_column_name(ContentObject :: PROPERTY_ID, $object_alias);
    ...
    function get_content_object_publication_attributes($object_id, $type = null, $offset = null, $count = null, $order_properties = null)
    {
        ...
        {
            $query = 'SELECT * FROM ' . $this->database->escape_table_name(AlexiaPublication :: get_table_name());
    ...
    {
        $query = 'SELECT * FROM ' . $this->database->escape_table_name(AlexiaPublication :: get_table_name()) . ' WHERE ' . $this->database->escape_column_name(AlexiaPublication :: PROPERTY_ID) . '=' . $this->quote($publication_id);
    ...
    {
        $where = $this->database->escape_column_name(AlexiaPublication :: PROPERTY_ID) . '=' . $publication_attr->get_id();
        $props = array();
        $props[$this->database->escape_column_name(AlexiaPublication :: PROPERTY_CONTENT_OBJECT)] = $publication_attr->get_publication_object_id();
        $this->database->get_connection()->loadModule('Extended');
        if ($this->database->get_connection()->extended->autoExecute($this->database->get_table_name(AlexiaPublication :: get_table_name()), $props, MDB2_AUTOQUERY_UPDATE, $where))
        {
            return true;
        }
        else
