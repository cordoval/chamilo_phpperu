...
class ForumManagerBrowserComponent extends ForumManager
{
    ...
    function create_table_forums($table, &$row, $parent)
    {
        ...
        while ($publication = $publications->next_result())
        {
            ...
            $title = '<a href="' . $this->get_url(array(
                        ForumManager :: PARAM_ACTION => ForumManager :: ACTION_VIEW,
                        ComplexDisplay :: PARAM_DISPLAY_ACTION => ForumDisplay :: ACTION_VIEW_FORUM,
                        ForumManager :: PARAM_PUBLICATION_ID => $publication->get_id())) . '">' . $forum->get_title() . '</a><br />' . Utilities :: truncate_string($forum->get_description());
    ...
    function get_forum_actions($publication, $first, $last)
    {
        ...
        {
            $delete = new ToolbarItem(Translation :: get('Delete', null, Utilities :: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_delete.png', $this->get_url(array(
                                ForumManager :: PARAM_PUBLICATION_ID => $publication->get_id(),
                                ForumManager :: PARAM_ACTION => ForumManager :: ACTION_DELETE)), ToolbarItem :: DISPLAY_ICON, true);
            ...
            if ($publication->is_hidden())
            {
                $toolbar->add_item(new ToolbarItem(Translation :: get('Show', null, Utilities :: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_invisible.png', $this->get_url(array(
                                    ForumManager :: PARAM_PUBLICATION_ID => $publication->get_id(),
                                    ForumManager :: PARAM_ACTION => ForumManager :: ACTION_TOGGLE_VISIBILITY)), ToolbarItem :: DISPLAY_ICON));
            }
            else
            {
            ...
            else
            {
                $toolbar->add_item(new ToolbarItem(Translation :: get('MoveUp', null, Utilities :: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_up.png', $this->get_url(array(
                                    ForumManager :: PARAM_PUBLICATION_ID => $publication->get_id(),
                                    ForumManager :: PARAM_ACTION => ForumManager :: ACTION_MOVE,
                                    ForumManager :: PARAM_MOVE => - 1)), ToolbarItem :: DISPLAY_ICON));
            }
            ...
            else
            {
                $toolbar->add_item(new ToolbarItem(Translation :: get('MoveDown', null, Utilities :: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_down.png', $this->get_url(array(
                                    ForumManager :: PARAM_PUBLICATION_ID => $publication->get_id())), ToolbarItem :: DISPLAY_ICON));
            }
            ...
            $toolbar->add_item(new ToolbarItem(Translation :: get('Edit', null, Utilities :: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_edit.png', $this->get_url(array(
                                ForumManager :: PARAM_PUBLICATION_ID => $publication->get_id(),
                                ForumManager :: PARAM_ACTION => ForumManager :: ACTION_EDIT)), ToolbarItem :: DISPLAY_ICON));
            ...
            if ($forum->get_locked())
            {
                ...
                $toolbar->add_item(new ToolbarItem(Translation :: get('Unlock', null, 'repository\content_object\forum'), Theme :: get_common_image_path() . 'action_unlock.png', $this->get_url($parameters), ToolbarItem :: DISPLAY_ICON));
            }
            else
            {
                $parameters[ForumManager :: PARAM_ACTION] = ForumManager :: ACTION_CHANGE_LOCK;
                $parameters[ForumManager :: PARAM_PUBLICATION_ID] = $publication->get_id();
                $toolbar->add_item(new ToolbarItem(Translation :: get('Lock', null, 'repository\content_object\forum'), Theme :: get_common_image_path() . 'action_lock.png', $this->get_url($parameters), ToolbarItem :: DISPLAY_ICON));
            }

            if (WebApplication :: is_active('gradebook'))
            {
                if (EvaluationManager :: retrieve_internal_item_by_publication(ForumManager :: APPLICATION_NAME, $publication->get_id()))
                    $toolbar->add_item(new ToolbarItem(Translation :: get('Evaluation', null, 'application\gradebook'), Theme :: get_common_image_path() . 'action_evaluation.png', $this->get_url(array(
                                        ForumManager :: PARAM_PUBLICATION_ID => $publication->get_id(),
                                        ForumManager :: PARAM_ACTION => ForumManager :: ACTION_EVALUATE)), ToolbarItem :: DISPLAY_ICON));
            }
        }
    ...
    function get_action_bar()
    {
        ...
        if (ForumRights :: is_allowed_in_forums_subtree(ForumRights :: PUBLISH_RIGHT))
        {
            $action_bar->add_common_action(new ToolbarItem(Translation :: get('Publish', null, Utilities :: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_publish.png', $this->get_url(array(
                                ForumManager :: PARAM_ACTION => ForumManager :: ACTION_CREATE)), ToolbarItem :: DISPLAY_ICON_AND_LABEL));
        }
