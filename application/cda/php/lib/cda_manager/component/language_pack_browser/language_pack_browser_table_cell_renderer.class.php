    ...
    function render_cell($column, $language_pack)
    {
        ...
        switch ($column->get_name())
        {
            case LanguagePack :: PROPERTY_NAME :
                ...
                if ($this->browser instanceof CdaManagerLanguagePacksBrowserComponent)
                {
    ...
    private function get_modification_links($language_pack)
    {
        ...
        else
        {
            if ($can_lock)
            {
                ...
                {
                    $toolbar->add_item(new ToolbarItem(Translation :: get('Lock'), Theme :: get_common_image_path() . 'action_lock.png', $this->browser->get_lock_language_pack_url($language_pack, $cda_language_id), ToolbarItem :: DISPLAY_ICON, true));
                }
                else
                {
                ...
                {
                    $toolbar->add_item(new ToolbarItem(Translation :: get('Unlock'), Theme :: get_common_image_path() . 'action_unlock.png', $this->browser->get_unlock_language_pack_url($language_pack, $cda_language_id), ToolbarItem :: DISPLAY_ICON));
                }
                else
                {
