...
require_once WebApplication :: get_application_class_lib_path('cda') . 'tables/cda_language_table/default_cda_language_table_cell_renderer.class.php';
    ...
    function render_cell($column, $cda_language)
    {
        ...
        switch ($column->get_name())
        {
            case CdaLanguage :: PROPERTY_ORIGINAL_NAME :
                if (! $this->browser->get_user()->is_platform_admin() && CdaRights :: is_allowed_in_languages_subtree(CdaRights :: VIEW_RIGHT, $cda_language->get_id(), 'cda_language'))
                {
            ...
            case CdaLanguage :: PROPERTY_ENGLISH_NAME :
                if ($this->browser instanceof CdaManagerCdaLanguagesBrowserComponent)
                {
