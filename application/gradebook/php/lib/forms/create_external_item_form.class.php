...
class CreateExternalItemForm extends FormValidator
{
    ...
    function build_basic_form($users_ids = null)
    {
        ...
        foreach ($this->users as $user)
        {
            ...
            if (!$this->evaluation_format->get_score_set())
            {
                ...
                $rule[$this->evaluation_format->get_evaluation_field_name() . $counter][] = array(Translation :: get('ValueShouldBeNumeric', null, Utilities::COMMON_LIBRARIES), 'numeric');
                $rule[$this->evaluation_format->get_evaluation_field_name() . $counter][] = array(Translation :: get('DecimalValueNotAllowed', null, Utilities::COMMON_LIBRARIES), $score_rule);
                $rule[$this->evaluation_format->get_evaluation_field_name() . $counter][] = array(Translation :: get('ScoreIsOutsideBoundaries', null, Utilities::COMMON_LIBRARIES), $boundaries_rule);
    ...
    function build_evaluation_format_element()
    {
        ...
        $this->add_textfield(ExternalItem :: PROPERTY_TITLE, Translation :: get('Title', null, Utilities::COMMON_LIBRARIES), true, array('size' => '100', 'id' => 'title', 'style' => 'width: 95%'));
