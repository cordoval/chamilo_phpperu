...
//                $conditions[] = new EqualityCondition(ContentObject::PROPERTY_TYPE, 'handbook');
    ...
    function retrieve_handbook_data_by_uuid($uuid)
    {
        $rdm = RepositoryDataManager::get_instance();
        $content_object_alias = 'co';
        $query = 'SELECT * FROM ' . $rdm->escape_table_name(ContentObject::get_table_name()) . ' AS ' . $content_object_alias;
        ...
        $type_alias = 'hi';
        $query .= ' JOIN ' . $rdm->escape_table_name(Handbook::get_type_name()) . ' AS ' . $type_alias . ' ON ' . $this->escape_column_name(ContentObject :: PROPERTY_ID, $content_object_alias) . ' = ' . $this->escape_column_name(ContentObject :: PROPERTY_ID, $type_alias);

        $query .= ' WHERE ' . $type_alias . '.' . Handbook::PROPERTY_UUID . ' = \'' . $uuid . '\'';

        $res = $this->query($query);
        $record = $res->fetchRow(\MDB2_FETCHMODE_ASSOC);
        if ($record)
        {
            $handbook[Handbook :: PROPERTY_ID] = $record[Handbook :: PROPERTY_ID];
            $handbook[Handbook :: PROPERTY_UUID] = $record[Handbook:: PROPERTY_UUID];
