...
class SurveyMailTesterForm extends FormValidator
{
    ...
    function __construct($parent, $user, $actions)
    {
       ...
       $this->addElement('text', self :: TO_ADDRESS_NAME, Translation :: get('SurveyToEmailAddressName'), array('size' => 80, 'value' => $user->get_firstname() . ' ' . $user->get_lastname()));
        $this->addRule(self :: TO_ADDRESS_NAME, Translation :: get('ThisFieldIsRequired'), 'required');
        $this->addElement('text', self :: TO_ADDRESS, Translation :: get('SurveyToEmailAddress'), array('size' => 80, 'value' => $user->get_email()));
