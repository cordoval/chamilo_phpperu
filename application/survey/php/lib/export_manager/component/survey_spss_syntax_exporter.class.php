...
use repository\content_object\survey\SurveyContextTemplate;
...
class SurveyExportManagerSurveySpssSyntaxExporterComponent extends SurveyExportManager
{
    ...
    private function get_questions()
    {
        ...
        foreach ($surveys as $survey)
        {
            ...
            {
                //test: old data without surveypage context relations
                //                $conditions = array();
    ...
    private function get_context_variables($ids)
    {
     ...
     //            $context_templates[$survey->get_context_template_id()] = $survey->get_context_template_for_level();
        ...
        //test: see what happens with default template
        $context_template = new SurveyContextTemplate();
        $context_template->set_name('test');
        $context_template->set_id(1);
        $context_templates[1] = $context_template;
