...
class LinkerManagerUpdaterComponent extends LinkerManager
{
    ...
    function run()
    {
        ...
        $form = new LinkForm(LinkForm :: TYPE_EDIT, $link, $this->get_url(array(
                LinkerManager :: PARAM_LINK_ID => $link->get_id())), $this->get_user());

        if ($form->validate())
        {
            $success = $form->update_link();
            $this->redirect($success ? Translation :: get('LinkUpdated', null, Utilities :: COMMON_LIBRARIES) : Translation :: get('LinkNotUpdated', null, Utilities :: COMMON_LIBRARIES), ! $success, array(
