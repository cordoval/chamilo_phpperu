...
class LaikaManagerViewerComponent extends LaikaManager
{
    ...
    function get_view_html()
    {
        ...
        if (LaikaRights :: is_allowed(LaikaRights :: RIGHT_VIEW, 'mailer', LaikaRights :: TYPE_LAIKA_COMPONENT))
        {
        ...
        $html[] = '<div class="title">' . Translation :: get('ResultsFrom') . ' ' . DatetimeUtilities :: format_locale_date(Translation :: get('DateTimeFormatLong', null, Utilities :: COMMON_LIBRARIES), $attempt->get_date()) . '</div>';
    ...
    function get_laika_attempt()
    {
        if (! isset($this->attempt))
        {
            ...
            $is_admin = LaikaRights :: is_allowed(LaikaRights :: RIGHT_VIEW, 'browser', LaikaRights :: TYPE_LAIKA_COMPONENT);
