 ...
 * @package application.lib.package.forms
 *
...
class TranslatorApplicationForm extends FormValidator
{
    ...
    private function build_form()
    {
    	...
    	$platform_setting = AdminDataManager :: get_instance()->retrieve_setting_from_variable_name('source_language', PackageManager :: APPLICATION_NAME);
    	$user_setting = UserDataManager :: get_instance()->retrieve_user_setting(Session :: get_user_id(), $platform_setting->get_id());

    	if ($user_setting)
    	{
    		$language = PackageDataManager :: get_instance()->retrieve_package_language($user_setting->get_value());
    ...
    function get_target_languages()
    {
    	...
    	$translator_applications = PackageDataManager :: get_instance()->retrieve_translator_applications($condition);
    ...
    function get_source_languages($exclude = array())
    {
    	if (count($exclude) > 0)
    	{
    		$condition = new NotCondition(new InCondition(PackageLanguage :: PROPERTY_ID, $exclude));
    	}
    	else
    	{
    		$condition = null;
    	...
    	$languages = PackageDataManager :: get_instance()->retrieve_package_languages($condition, null, null, array(new ObjectTableOrder(PackageLanguage :: PROPERTY_ORIGINAL_NAME)));
    	$options = array();
    ...
    function setDefaults($defaults = array ())
    {
    	...
    	$user_language = PackageDataManager :: get_instance()->retrieve_package_languages(new EqualityCondition(PackageLanguage :: PROPERTY_ENGLISH_NAME, $user_language_text), null, 1)->next_result();
    	...
    	else
    	{
    		...
    		{
    			$source_language = LocalSetting :: get('source_language', PackageManager :: APPLICATION_NAME);
    		}
    	}
    ...
    function notify($applications, $source_language)
    {
    	...
    	foreach($applications as $application => $language)
    	{
    		...
    		$link = Path :: get(WEB_PATH) . Redirect :: get_link('package', array(Application :: PARAM_ACTION => PackageManager :: ACTION_ACTIVATE_TRANSLATOR_APPLICATION,
    											  PackageManager :: PARAM_TRANSLATOR_APPLICATION => $application));

	    	$html[] = Translation :: get('DestinationLanguage') . ': <a href="' . $link . '">' . $language_name . '</a>';

	    	$link = Path :: get(WEB_PATH) . Redirect :: get_link('package', array(Application :: PARAM_ACTION => PackageManager :: ACTION_BROWSE_VARIABLE_TRANSLATIONS));
	    	...
	    	$subject = '[PACKAGE] ' . Translation :: get('UserAppliedForTranslator');
    ...
    {
    	$language = PackageDataManager :: get_instance()->retrieve_package_language($language_id);
    	return $language;
    }

	function get_moderator_emails($package_language)
	{
		$moderators = PackageRights :: get_allowed_users(PackageRights :: EDIT_RIGHT, $package_language->get_id(), $package_language->get_table_name());
