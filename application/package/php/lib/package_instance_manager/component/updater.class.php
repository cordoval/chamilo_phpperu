...
class PackageInstanceManagerUpdaterComponent extends PackageInstanceManager implements AdministrationComponent
{
    ...
    function run()
    {
        ...
        $form = new PackageForm(PackageForm :: TYPE_EDIT, $package, $this->get_url(array(
                self :: PARAM_PACKAGE_ID => $package->get_id())), $this->get_user());
        if ($form->validate())
        {
            ...
            $this->redirect($message, ! $success, array(
                    PackageInstanceManager :: PARAM_PACKAGE_INSTANCE_ACTION => self :: ACTION_BROWSE));
        }
        else
        {
            $this->display_header();
    ...
    function add_additional_breadcrumbs(BreacrumbTrail $breadcrumbtrail)
    {
        ...
        $breadcrumbtrail->add(new Breadcrumb($this->get_url(array(
                self :: PARAM_PACKAGE_INSTANCE_ACTION => PackageInstanceManager :: ACTION_BROWSE)), Translation :: get('PackageInstanceManagerPackageBrowserComponent')));
    }
