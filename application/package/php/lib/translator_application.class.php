...
class TranslatorApplication extends DataClass
{
    ...
    function get_source_language()
    {
    	...
    	return $cdm->retrieve_package_language($this->get_source_language_id());
    }
    
    function get_destination_language()
    {
    	$cdm = PackageDataManager :: get_instance();
    	return $cdm->retrieve_package_language($this->get_destination_language_id());
    }
    
    function activate()
    {
    	$source_setting = AdminDataManager :: get_instance()->retrieve_setting_from_variable_name('source_language', PackageManager :: APPLICATION_NAME);
    	...
    	if(!$source_user_setting)
    	{
   		...
   		$location = PackageRights :: get_location_id_by_identifier_from_languages_subtree(PackageRights :: TYPE_LANGUAGE, $this->get_destination_language_id());
    	$success = RightsUtilities :: set_user_right_location_value(PackageRights :: VIEW_RIGHT, $this->get_user_id(), $location, true);
    ...
    function deactivate()
    {
    	...
    	$location = PackageRights :: get_location_id_by_identifier_from_languages_subtree(PackageRights :: TYPE_LANGUAGE, $this->get_destination_language_id());
    	$success = RightsUtilities :: set_user_right_location_value(PackageRights :: VIEW_RIGHT, $this->get_user_id(), $location, false);
