...
abstract class TranslationImporter
{
    ...
    private function import_translations($root)
    {
    	...
    	foreach($languages as $language)
    	{
    		...
    		$is_translator = PackageRights :: is_allowed_in_languages_subtree(PackageRights :: VIEW_RIGHT, $system_language->get_id(), 'package_language');
			$is_moderator = PackageRights :: is_allowed_in_languages_subtree(PackageRights :: EDIT_RIGHT, $system_language->get_id(), 'package_language');
	...
	private function get_language($language_name)
	{
		...
		$condition = new EqualityCondition(PackageLanguage :: PROPERTY_ENGLISH_NAME, $language_name);
		
		$language = $dm->retrieve_package_languages($condition)->next_result();
		if(!$language && $this->get_option(self :: OPTION_CREATE_NEW_LANGUAGES))
		{
			$language = new PackageLanguage();
