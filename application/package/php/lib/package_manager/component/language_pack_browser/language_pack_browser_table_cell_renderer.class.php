 ...
 */
require_once WebApplication :: get_application_class_lib_path('package') . 'package_manager/component/language_pack_browser/language_pack_browser_table_column_model.class.php';
require_once WebApplication :: get_application_class_lib_path('package') . 'tables/language_pack_table/default_language_pack_table_cell_renderer.class.php';
...
class LanguagePackBrowserTableCellRenderer extends DefaultLanguagePackTableCellRenderer
{
    ...
    function render_cell($column, $language_pack)
    {
        ...
        switch ($column->get_name())
        {
            case LanguagePack :: PROPERTY_NAME :
                ...
                if ($this->browser instanceof PackageManagerLanguagePacksBrowserComponent)
                {
                    $url = $this->browser->get_browse_variable_translations_url(Request :: get(PackageManager :: PARAM_PACKAGE_LANGUAGE), $language_pack->get_id());
                }
                else
                {
            ...
            case Translation :: get('TranslationProgress') :
                $percentage = $this->browser->get_progress_for_language_pack($language_pack, $this->browser->get_package_language());
    ...
    {
        $package_language_id = $this->browser->get_package_language();
        $can_lock = PackageRights :: is_allowed_in_languages_subtree(PackageRights :: EDIT_RIGHT, $package_language_id, 'package_language');
        $can_translate = PackageRights :: is_allowed_in_languages_subtree(PackageRights :: VIEW_RIGHT, $package_language_id, 'package_language');
        ...
        else
        {
            if ($can_lock)
            {
                if ($this->browser->can_language_pack_be_locked($language_pack, $package_language_id))
                {
                    $toolbar->add_item(new ToolbarItem(Translation :: get('Lock'), Theme :: get_common_image_path() . 'action_lock.png', $this->browser->get_lock_language_pack_url($language_pack, $package_language_id), ToolbarItem :: DISPLAY_ICON, true));
                }
                else
                {
                ...
                if ($this->browser->can_language_pack_be_unlocked($language_pack, $package_language_id))
                {
                    $toolbar->add_item(new ToolbarItem(Translation :: get('Unlock'), Theme :: get_common_image_path() . 'action_unlock.png', $this->browser->get_unlock_language_pack_url($language_pack, $package_language_id), ToolbarItem :: DISPLAY_ICON));
                }
                else
                {
            ...
            if ($can_translate || $can_lock)
            {
                ...
                $translation = $this->browser->retrieve_first_untranslated_variable_translation($package_language_id, $language_pack->get_id(), $status);

                if ($translation)
                {
                    $toolbar->add_item(new ToolbarItem(Translation :: get('TranslateFirstEmptyTranslation'), Theme :: get_common_image_path() . 'action_quickstart.png', $this->browser->get_update_variable_translation_url($translation), ToolbarItem :: DISPLAY_ICON));
                }
            }
        }
