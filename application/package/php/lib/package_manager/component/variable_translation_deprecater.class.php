 ...
 */
class PackageManagerVariableTranslationDeprecaterComponent extends PackageManager
{
	/**
	...
	function run()
	{
		...
		if (!empty ($ids))
		{
			...
			foreach ($ids as $id)
			{
				...
				elseif (!$variable_translation->deprecate())
				{
					$failures++;
				}
			}
			...
			if ($failures)
			{
				if (count($ids) == 1)
				{
					$message = 'SelectedVariableTranslationNotDeprecated';
				}
				else
				{
					$message = 'SelectedVariableTranslationsNotDeprecated';
				}
			}
			else
			{
				if (count($ids) == 1)
				{
					$message = 'SelectedVariableTranslationDeprecated';
				}
				else
				{
					$message = 'SelectedVariableTranslationsNotDeprecated';
	...
	function add_additional_breadcrumbs(BreacrumbTrail $breadcrumbtrail)
    {
    	...
    	$breadcrumbtrail->add_help('package_variable_translations_deprecater');
    }
