    ...
    function get_condition()
    {
        ...
        if (! $user->is_platform_admin())
        {
            ...
            $dates[] = new AndCondition(array(
                    new InequalityCondition(GutenbergPublication :: PROPERTY_FROM_DATE, InequalityCondition :: GREATER_THAN_OR_EQUAL, time()),
                    new InequalityCondition(GutenbergPublication :: PROPERTY_TO_DATE, InequalityCondition :: LESS_THAN_OR_EQUAL, time())));
    ...
    function get_action_bar()
    {
        if (! isset($this->action_bar))
        {
            ...
            $this->action_bar->add_common_action(new ToolbarItem(Translation :: get('Publish', null, Utilities :: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_publish.png', $this->get_url(array(
                    Application :: PARAM_ACTION => GutenbergManager :: ACTION_CREATE_PUBLICATION))));
            ...
            if (count($renderers) > 1)
            {
                ...
                {
                    $this->action_bar->add_tool_action(new ToolbarItem(Translation :: get(Utilities :: underscores_to_camelcase($renderer) . 'View', null, Utilities :: COMMON_LIBRARIES), Theme :: get_image_path() . 'view_' . $renderer . '.png', $this->get_url(array(
                            GutenbergManager :: PARAM_RENDERER => $renderer)), ToolbarItem :: DISPLAY_ICON_AND_LABEL));
                }
            }
