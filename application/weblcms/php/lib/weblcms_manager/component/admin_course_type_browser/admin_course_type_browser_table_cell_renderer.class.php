...
class AdminCourseTypeBrowserTableCellRenderer extends DefaultCourseTypeTableCellRenderer
{
    ...
    private function get_modification_links($course_type)
    {
        ...
        $toolbar->add_item(new ToolbarItem(($course_type->get_active() == 1) ? Translation :: get('Deactivate', null ,Utilities:: COMMON_LIBRARIES) : Translation :: get('Activate', null ,Utilities:: COMMON_LIBRARIES), ($course_type->get_active() == 1) ? Theme :: get_common_image_path() . 'action_visible.png' : Theme :: get_common_image_path() . 'action_invisible.png', $this->browser->get_change_active_url('course_type', $course_type->get_id()), ToolbarItem :: DISPLAY_ICON));

        $toolbar->add_item(new ToolbarItem(Translation :: get('Edit', null ,Utilities:: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_edit.png', $this->browser->get_course_type_editing_url($course_type), ToolbarItem :: DISPLAY_ICON));

        $toolbar->add_item(new ToolbarItem(Translation :: get('Delete', null ,Utilities:: COMMON_LIBRARIES), Theme :: get_common_image_path() . 'action_delete.png', $this->browser->get_course_type_deleting_url($course_type), ToolbarItem :: DISPLAY_ICON, true));
