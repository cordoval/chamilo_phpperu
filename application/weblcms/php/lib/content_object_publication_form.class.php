...
class ContentObjectPublicationForm extends FormValidator
{
    ...
    function build_form()
    {
        ...
        $this->categories[0] = Translation :: get('Root', null ,Utilies:: COMMON_LIBRARIES);
        ...
        if (count($this->categories) > 1)
        {
            ...
            if ($this->tool->is_allowed(WeblcmsRights :: EDIT_RIGHT))
            {
                ...
                $this->addElement('select', self :: PARAM_CATEGORY_ID, Translation :: get('Category', null ,Utilies:: COMMON_LIBRARIES), $this->categories);
            }
            else
            {
        ...
        $locale['Searching'] = Translation :: get('Searching', null ,Utilies:: COMMON_LIBRARIES);
        $locale['NoResults'] = Translation :: get('NoResults', null ,Utilies:: COMMON_LIBRARIES);
        $locale['Error'] = Translation :: get('Error', null ,Utilies:: COMMON_LIBRARIES);
        ...
        $this->add_receivers(self :: PARAM_TARGET, Translation :: get('PublishFor', null ,Utilies:: COMMON_LIBRARIES), $attributes, 'Everybody', $legend);

        $this->add_forever_or_timewindow();
        $this->addElement('checkbox', self :: PARAM_HIDDEN, Translation :: get('Hidden', null ,Utilies:: COMMON_LIBRARIES));
    ...
    function add_footer()
    {
        $buttons[] = $this->createElement('style_submit_button', 'submit', Translation :: get('Publish', null ,Utilies:: COMMON_LIBRARIES), array('class' => 'positive publish'));
     ...
     //$this->addElement('submit', 'submit', Translation :: get('Ok', null ,Utilies:: COMMON_LIBRARIES));
    }
