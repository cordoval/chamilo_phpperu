...
class ActionSelectionMaintenanceWizardPage extends MaintenanceWizardPage
{
	...
	/**
	 * Export a course's content to the imscp format.
	 */
	const ACTION_EXPORT_CP = 501;
	/**
	 * Import a course's content from the imscp format.
	 */
	const ACTION_IMPORT_CP = 502;
	...
	function buildForm()
	{
		...
		$available = $this->is_available(self :: ACTION_EXPORT_CP) ? null : 'disabled';
	...
	private function is_available($action)
	{
		...
		switch ($action)
		{
			...
			case self :: ACTION_EMPTY :
				...
				if ($dm->count_content_object_publications($condition) == 0)
				{
					return false;
				}
				return true;
