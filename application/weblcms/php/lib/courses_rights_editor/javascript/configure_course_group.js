( function($) {
	...
	var collapseItem = function(e) {
		...
		$.post("./application/weblcms/php/courses_rights_editor/javascript/ajax/course_group_right_location.php", {
			rights : id,
			locations: locations
			}, function(result)
			{
				if (result)
				{
					var newClass = $.ajax({
							type: "POST",
							url: "./application/weblcms/php/courses_rights_editor/javascript/ajax/course_group_right_location_class.php",
							data: { rights : id },
							async: false
						}).responseText;
