    ...
    function display_new_items($items)
    {
        ...
        if (count($items) > 0)
        {
            ...
            foreach ($items as $item)
            {
                ...
                $html[] = '<li><a href="' . $weblcms->get_link(array(
                        'go' => 'courseviewer', 'application' => 'weblcms', 'tool' => 'announcement', 'tool_action' => 'view', Tool :: PARAM_PUBLICATION_ID => $item['id'], 'course' => $item['course'])) . '">' . $item['title'] . '</a>';
                $html[] = '</li>';
            }
            $html[] = '</ul>';
        }
